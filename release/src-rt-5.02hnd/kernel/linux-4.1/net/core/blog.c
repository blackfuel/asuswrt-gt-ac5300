                       ...
                       unsigned long param1, unsigned long param2 )
{
    ...
    switch ( request )
    {
        ...
        case NETIF_PUT_STATS:
        {
            ...
            blog_print("dev_p<%p> rx_pkt<%lu> rx_byte<%lu> tx_pkt<%lu>"
                       " tx_byte<%lu> multicast<%lu>", dev_p,
        ...
        case LINK_XMIT_FN:
        {
            ...
            {
                ret = (uint32_t)(rx_dev_p->netdev_ops->ndo_dpi_enqueue);
            }
            else
#endif
            {
        ...
        {
            if ( likely(blog_fa_hook_g != (BlogFaHook_t)NULL) )
            {
                struct net_device * rx_dev_p = (struct net_device *)(param1);

                blog_fa_hook_g(net_p, rx_dev_p->priv_flags & IFF_WANDEV,
        ...
        {
            if ( likely(blog_fd_hook_g != (BlogFdHook_t)NULL) )
            {
                struct net_device * rx_dev_p = (struct net_device *)(param1);

                blog_fd_hook_g(net_p, rx_dev_p->priv_flags & IFF_WANDEV,
                               (BlogFlowEventType_t)(param2));
            }
...
void blog_bind_flow_event( BlogFaHook_t blog_fa, BlogFdHook_t blog_fd,
                           BlogBind_t bind )
{
    blog_print( "Bind fa[<%08x>] fd[<%08x>] bind[<%u>]",
                (int)blog_fa, (int)blog_fd, (uint8_t)bind.hook_info );
