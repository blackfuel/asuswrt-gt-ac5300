...
destroy_conntrack(struct nf_conntrack *nfct)
{
	...
	if (test_bit(IPS_IQOS_BIT,&ct->status))
	{
		...
		iqos_rem_L4port(IQOS_IPPROTO_UDP, ct->tuplehash[IP_CT_DIR_ORIGINAL].tuple.dst.u.udp.port, IQOS_ENT_DYN);
		iqos_rem_L4port(IQOS_IPPROTO_UDP, ct->tuplehash[IP_CT_DIR_REPLY].tuple.dst.u.udp.port, IQOS_ENT_DYN);

		//printk("remove iqos port :%u\n", ct->tuplehash[IP_CT_DIR_ORIGINAL].tuple.dst.u.udp.port);
