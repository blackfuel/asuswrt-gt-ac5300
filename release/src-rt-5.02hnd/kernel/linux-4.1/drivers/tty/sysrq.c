...
#ifdef CATHY_TRACE
#define BT_DEPTH 2
#define IRQ_REC_HISTORY 3
unsigned int trace_flag = 0;
EXPORT_SYMBOL(trace_flag);

struct irqdata {
	unsigned int magic;
	int on_idx;
	int off_idx;
	unsigned long on_ret_ip[IRQ_REC_HISTORY][BT_DEPTH];
	unsigned long on_locks[IRQ_REC_HISTORY];
	unsigned long off_ret_ip[IRQ_REC_HISTORY][BT_DEPTH];
	unsigned long off_locks[IRQ_REC_HISTORY];
};

#define IRQ_REC_NEXT_IDX(idx) (((idx) == ((IRQ_REC_HISTORY) - 1)) ? 0 : ((idx) + 1))

struct irqdata trace_hardirqs[NR_CPUS];

void cathy_trace_irqs_on(unsigned long lock, unsigned long this_ip, unsigned long ret_ip, unsigned int flag)
{
	struct irqdata *trace;
	int idx;
	int cpu;
	...
	trace = &trace_hardirqs[cpu];
	idx = trace->on_idx;
	trace->on_ret_ip[idx][0] = this_ip;
	trace->on_ret_ip[idx][1] = ret_ip;
	trace->on_locks[idx] = lock;
	trace->on_idx = IRQ_REC_NEXT_IDX(idx);
}
EXPORT_SYMBOL(cathy_trace_irqs_on);

void cathy_trace_irqs_off(unsigned long lock, unsigned long this_ip, unsigned long ret_ip, unsigned int flag)
{
	struct irqdata *trace;
	int idx;
	int cpu;
	...
	trace = &trace_hardirqs[cpu];
	idx = trace->off_idx;
	trace->off_ret_ip[idx][0] = this_ip;
	trace->off_ret_ip[idx][1] = ret_ip;
	trace->off_locks[idx] = lock;
	trace->off_idx = IRQ_REC_NEXT_IDX(idx);
}
EXPORT_SYMBOL(cathy_trace_irqs_off);

static DEFINE_SPINLOCK(trace_lock);

void dump_trace_irqs(void)
{
	struct irqdata *trace;
	int i, j, k;
	int idx;
	...
	for_each_online_cpu(i) {
		trace = &trace_hardirqs[i];

		printk("*****************************************************\n");
		printk("dump_traceirq CPU %d\n", i);
		/* irq on history */
		idx = trace->on_idx;
		for (j = 0; j < IRQ_REC_HISTORY; j++) {
			printk("  irq on history idx %d locks 0x%lx\n", idx, trace->on_locks[idx]);
			for (k = 0; k < BT_DEPTH; k++) {
				print_ip_sym(trace->on_ret_ip[idx][k]);
			}
			idx = IRQ_REC_NEXT_IDX(idx);
		}
		printk("=====================================================\n");
		/* irq off history */
		idx = trace->off_idx;
		for (j = 0; j < IRQ_REC_HISTORY; j++) {
			printk("  irq off history idx %d lock 0x%lx\n", idx, trace->off_locks[idx]);
			for (k = 0; k < BT_DEPTH; k++) {
				print_ip_sym(trace->off_ret_ip[idx][k]);
			}
			idx = IRQ_REC_NEXT_IDX(idx);
		}
...
EXPORT_SYMBOL(dump_trace_irqs);

void init_irq_trace(void)
{
	unsigned long flags;
	int i;
	void *trace_hardirqs_phys_addr;

	spin_lock_irqsave(&trace_lock, flags);
	trace_hardirqs_phys_addr = (void *)virt_to_phys((void *)&trace_hardirqs);

	printk(KERN_ERR "*****************************************************\n");
	printk(KERN_ERR "trace_hardirqs virt addr 0x%p phys addr 0x%p (size %ld bytes = %ld words)\n",
		&trace_hardirqs, trace_hardirqs_phys_addr, sizeof(trace_hardirqs), sizeof(trace_hardirqs)/4);
	printk(KERN_ERR "trace_flag virt addr 0x%p\n", &trace_flag);
	printk(KERN_ERR "*****************************************************\n");
	memset(&trace_hardirqs, 0, sizeof(trace_hardirqs));
	for (i = 0; i < NR_CPUS; i++) {
		trace_hardirqs[i].magic = 0x12345678;
	}
	trace_flag = TRACE_FLAG_SPINLOCK_0 | TRACE_FLAG_SPINLOCK_1;
