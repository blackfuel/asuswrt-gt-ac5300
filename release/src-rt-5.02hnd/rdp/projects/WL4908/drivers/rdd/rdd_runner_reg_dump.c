...
void dump_RDD_DS_SERVICE_QUEUE_DESCRIPTOR(bdmf_session_handle session, unsigned char *p)
{
	unsigned int r;
	bdmf_session_print(session, "  Register DS_SERVICE_QUEUE_DESCRIPTOR\n");
...
void dump_RDD_CPU_TX_DESCRIPTOR_US_FAST(bdmf_session_handle session, unsigned char *p)
{
	...
	FIELD_MREAD_32((uint8_t *)p + 0, 0, 20, r);
	bdmf_session_print(session, "\treserved1                = 0x%08x", (unsigned int)r);
	bdmf_session_print(session, "\n");
...
void dump_RDD_FC_MCAST_PORT_CONTEXT_ENTRY(bdmf_session_handle session, unsigned char *p)
{
	...
	FIELD_MREAD_16((uint8_t *)p, 5, 10, r);
	bdmf_session_print(session, "\treserved2                = 0x%08x", (unsigned int)r);
	bdmf_session_print(session, "\n");
...
void dump_RDD_DS_SERVICE_TM_DESCRIPTOR_ENTRY(bdmf_session_handle session, unsigned char *p)
{
	unsigned int r;
	bdmf_session_print(session, "  Register DS_SERVICE_TM_DESCRIPTOR_ENTRY\n");
	...
	FIELD_MREAD_8((uint8_t *)p + 2, 7, 1, r);
	bdmf_session_print(session, "\tscheduler_method_NOT_USED= 0x%08x", (unsigned int)r);
	bdmf_session_print(session, "\n");
...
void dump_RDD_FC_COMMAND_PRIMITIVE_ENTRY(bdmf_session_handle session, unsigned char *p)
{
	unsigned int r;
	bdmf_session_print(session, "  Register FC_COMMAND_PRIMITIVE_ENTRY\n");
...
void dump_RDD_US_INGRESS_CLASSIFICATION_CONTEXT_ENTRY(bdmf_session_handle session, unsigned char *p)
{
	...
	FIELD_MREAD_8((uint8_t *)p + 4, 0, 6, r);
	bdmf_session_print(session, "\treserved1                = 0x%08x", (unsigned int)r);
	bdmf_session_print(session, "\n");
...
void dump_RDD_DHD_RADIO_INSTANCE_COMMON_A_ENTRY(bdmf_session_handle session, unsigned char *p)
{
	...
	MREAD_32((uint8_t *)p, r);
	bdmf_session_print(session, "\tds_dhd_doorbell          = 0x%08x", (unsigned int)r);
	bdmf_session_print(session, "\n");
...
void dump_RDD_L4_DST_PORT_ENTRY(bdmf_session_handle session, unsigned char *p)
{
	unsigned int r;
	bdmf_session_print(session, "  Register L4_DST_PORT_ENTRY\n");

	MREAD_16((uint8_t *)p, r);
	bdmf_session_print(session, "\tport                     = 0x%08x", (unsigned int)r);
	bdmf_session_print(session, "\n");

}

void dump_RDD_L4_DST_PORT_CONTEXT(bdmf_session_handle session, unsigned char *p)
{
	unsigned int r;
	bdmf_session_print(session, "  Register L4_DST_PORT_CONTEXT\n");

	FIELD_MREAD_8((uint8_t *)p, 7, 1, r);
	bdmf_session_print(session, "\tis_TCP                   = 0x%08x", (unsigned int)r);
	bdmf_session_print(session, "\n");

	FIELD_MREAD_8((uint8_t *)p, 6, 1, r);
	bdmf_session_print(session, "\tis_static                = 0x%08x", (unsigned int)r);
	bdmf_session_print(session, "\n");

	FIELD_MREAD_8((uint8_t *)p, 0, 6, r);
	bdmf_session_print(session, "\trefcnt                   = 0x%08x", (unsigned int)r);
	bdmf_session_print(session, "\n");
...
void dump_RDD_DHD_RADIO_INSTANCE_COMMON_B_ENTRY(bdmf_session_handle session, unsigned char *p)
{
	...
	MREAD_32((uint8_t *)p, r);
	bdmf_session_print(session, "\tus_dhd_doorbell          = 0x%08x", (unsigned int)r);
	bdmf_session_print(session, "\n");
	...
	FIELD_MREAD_16((uint8_t *)p + 46, 1, 15, r);
	bdmf_session_print(session, "\treserved1                = 0x%08x", (unsigned int)r);
	bdmf_session_print(session, "\n");

	FIELD_MREAD_8((uint8_t *)p + 47, 0, 1, r);
	bdmf_session_print(session, "\tadd_llcsnap_header       = 0x%08x", (unsigned int)r);
	bdmf_session_print(session, "\n");

	MREAD_32((uint8_t *)p + 48, r);
	bdmf_session_print(session, "\ttx_post_mgmt_fr_base_ptr_rdd_only_low= 0x%08x", (unsigned int)r);
	bdmf_session_print(session, "\n");

	MREAD_32((uint8_t *)p + 52, r);
	bdmf_session_print(session, "\ttx_post_mgmt_fr_base_ptr_rdd_only_high= 0x%08x", (unsigned int)r);
	bdmf_session_print(session, "\n");
...
void dump_RDD_FC_UCAST_FLOW_CONTEXT_ENTRY(bdmf_session_handle session, unsigned char *p)
{
	...
	bdmf_session_print(session, "\n");
	MREAD_8((uint8_t *)p + 96, r);
	bdmf_session_print(session, "\tvalid                    = 0x%08x", (unsigned int)r);
	bdmf_session_print(session, "\n");

	FIELD_MREAD_8((uint8_t *)p + 97, 4, 4, r);
	bdmf_session_print(session, "\tcommand_list_length_64   = 0x%08x", (unsigned int)r);
	bdmf_session_print(session, "\n");

	FIELD_MREAD_8((uint8_t *)p + 97, 1, 3, r);
	bdmf_session_print(session, "\treserved4                = 0x%08x", (unsigned int)r);
	bdmf_session_print(session, "\n");

	FIELD_MREAD_8((uint8_t *)p + 97, 0, 1, r);
...
void dump_RDD_FC_NATC_UCAST_FLOW_CONTEXT_ENTRY(bdmf_session_handle session, unsigned char *p)
{
	...
	FIELD_MREAD_8((uint8_t *)p + 8, 3, 2, r);
	bdmf_session_print(session, "\treserved1                = 0x%08x", (unsigned int)r);
	bdmf_session_print(session, "\n");
...
void dump_RDD_FC_NATC_MCAST_FLOW_CONTEXT_ENTRY(bdmf_session_handle session, unsigned char *p)
{
	...
	FIELD_MREAD_8((uint8_t *)p + 10, 3, 2, r);
	bdmf_session_print(session, "\tlag_port                 = 0x%08x", (unsigned int)r);
	bdmf_session_print(session, "\n");
