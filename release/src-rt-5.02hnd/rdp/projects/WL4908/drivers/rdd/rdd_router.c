...
static int f_rdd_free_context_entry ( uint32_t  context_entry_index );

int rdd_fc_accel_mode_get(uint32_t *xo_accel_mode)
{
...
int rdd_fc_accel_mode_write(uint32_t xi_accel_mode)
{
    uint8_t *accel_mode_config_ptr;
...
#else
    nat_cache_table_address = NAT_CACHE_TABLE_ADDRESS;
#endif
...
#else
    context_table_address = NATC_CONTEXT_TABLE_ADDRESS;
#endif
...
#else
    context_cont_table_address = CONTEXT_CONTINUATION_TABLE_ADDRESS;
#endif
...
static int rdd_nat_cache_submit_command(uint32_t command, uint32_t *keyword, uint32_t *hit_count, uint32_t *byte_count)
{
                                       ...
                                       uint32_t                     is_new_entry )
{
    ...
    if( context_entry->fc_ucast_flow_context_entry.multicast_flag == 0 )
    {
        if(is_new_entry)
        {
            ...
            continuation_flag= ( context_entry->fc_ucast_flow_context_entry.command_list_length_64*8 > RDD_FC_NATC_UCAST_FLOW_CONTEXT_ENTRY_COMMAND_LIST_NUMBER + 4 ) ? 1 : 0;
            ...
            RDD_FC_NATC_UCAST_FLOW_CONTEXT_ENTRY_IS_UNICAST_WFD_NIC_WRITE(context_entry->fc_ucast_flow_context_entry.is_unicast_wfd_nic, natc_context_entry_ptr);
            ...
            else
            {
                ...
                RDD_FC_UCAST_FLOW_CONTEXT_ETH_XTM_ENTRY_LAG_PORT_WRITE(context_entry->fc_ucast_flow_context_eth_xtm_entry.lag_port , natc_context_entry_ptr);
            }
            ...
            {
     	        int remaining_length = context_entry->fc_ucast_flow_context_entry.command_list_length_64*8 - RDD_FC_NATC_UCAST_FLOW_CONTEXT_ENTRY_COMMAND_LIST_NUMBER;
                               ...
                               rdpa_traffic_dir                 direction )
{
    ...
    entry_bytes[ 1 ] = 0;
    entry_bytes[ 2 ] = 0;
    entry_bytes[ 3 ] = add_connection->lookup_entry->prot;
    ...
    do
    {
        ...
        RDD_NAT_CACHE_LKP_ENTRY_PROTOCOL_WRITE ( add_connection->lookup_entry->prot, nat_cache_lkp_entry_ptr );
...
int rdd_connection_entry_delete ( bdmf_index  flow_entry_index )
{
    ...
    if (entry_index < RDD_NAT_CACHE_EXTENSION_TABLE_SIZE) {
        nat_cache_lkp_entry_ptr = &(nat_cache_table_ptr->entry[RDD_NAT_CACHE_TABLE_SIZE + entry_index]);
                               ...
                               bdmf_index          *flow_entry_index )
{
    ...
    nat_cache_lkp_entry_ptr = &( nat_cache_table_ptr->entry[ entry_index ] );
    ...
    if( !multicast_flag )
    {
        ...
        if ( valid )
        {
            ...
            if ( flow_entry_index == context_index )
            {
                ...
                RDD_FC_NATC_UCAST_FLOW_CONTEXT_ENTRY_IS_UNICAST_WFD_NIC_READ(context_entry->fc_ucast_flow_context_entry.is_unicast_wfd_nic, context_entry_ptr);
