...
#define DRV_BBH_DDR_BUFFER_SIZE_KB  DRV_BBH_DDR_BUFFER_SIZE_4_KB
#define FPM_POOL_SIZE_MAX 32  /* MB, 8K buffer * 4KB/buffer */
#else
#define DRV_BBH_DDR_BUFFER_SIZE_KB  DRV_BBH_DDR_BUFFER_SIZE_2_KB
#define FPM_POOL_SIZE_MAX 16  /* MB, 8K buffer * 2KB/buffer */
#endif
...
{
    {
        ...
        DRV_RDD_IH_CLASS_WAN_INDEX,
        {
            ...
            DRV_RDD_IH_CLASS_13_CLASS_SEARCH_4,
...
{
    if (DRV_BBH_DDR_BUFFER_SIZE_KB == DRV_BBH_DDR_BUFFER_SIZE_2_KB)
    {
        return VAL_2K;
    }
    else if (DRV_BBH_DDR_BUFFER_SIZE_KB == DRV_BBH_DDR_BUFFER_SIZE_4_KB)
    {
        return VAL_4K;
    }
    else if (DRV_BBH_DDR_BUFFER_SIZE_KB == DRV_BBH_DDR_BUFFER_SIZE_16_KB)
    {
        return VAL_16K;
    }
    return 0; /* Error */
...
static void init_single_bbh(DRV_BBH_PORT_INDEX bbh)
{
    ...
    bbh_rx_configuration.runner_0_task_normal_queue = BBH_PORT_IS_WAN(bbh) ? WAN1_FILTERS_AND_CLASSIFICATION_THREAD_NUMBER : 0;
    bbh_rx_configuration.runner_0_task_direct_queue = 0;
    bbh_rx_configuration.runner_1_task_normal_queue = BBH_PORT_IS_WAN(bbh) ? 0 : LAN_DISPATCH_THREAD_NUMBER + (bbh - DRV_BBH_EMAC_0);
...
static void configure_runner(void)
{
    ...
    natc_key_context_size = sizeof(RDD_FC_NATC_UCAST_FLOW_CONTEXT_ENTRY_DTS) * RDD_CONTEXT_TABLE_SIZE;
    context_cont_size = sizeof(RDD_CONTEXT_CONTINUATION_ENTRY_DTS) * RDD_CONTEXT_TABLE_SIZE;
    rdd_init_params.runner_nat_cache_key_ptr = rdd_init_params.ddr_fm_ptr +  NAT_CACHE_TABLE_ADDRESS;
    rdd_init_params.runner_nat_cache_context_ptr = rdd_init_params.ddr_fm_ptr + NATC_CONTEXT_TABLE_ADDRESS;
    rdd_init_params.runner_context_cont_ptr = rdd_init_params.ddr_fm_ptr + CONTEXT_CONTINUATION_TABLE_ADDRESS;
...
static void natcache_drv_init(void)
{
    ...
    uint32_t key_base = (uint32_t)(uintptr_t)p_dpi_cfg->rdp_ddr_fm_phys + NAT_CACHE_TABLE_ADDRESS;
    uint32_t result_base = (uint32_t)(uintptr_t)p_dpi_cfg->rdp_ddr_fm_phys + NATC_CONTEXT_TABLE_ADDRESS;
...
static void enable_source_ports(void)
{
    ...
    sbpm_sp_enable.gpon_or_eth5_sp_enable = 0;
...
#ifdef __KERNEL__
    soc_base_address = (uint8_t*)ioremap(RDP_PHYS_BASE, RDP_SIZE);
...
static int check_reserved_memory(void)
{
    ...
    // TODO! implement this
    // based on rdp_ddr_tm_size and token size, and calculate the number of buffer supported
    // each buffer requires 512 byte multicast header memory.
    // total rdp_ddr_fm_size must be larger or equal to RDP_DDR_DATA_STRUCTURES_SIZE +
    // the multicast required memory
    ...
    if (p_dpi_cfg->runner_ddr_fm_size < sz_mb_required) {
        printk("You need %d MB memory, but only have reserved %d\n",
...
static void init_unimacs(void)
{
    ...
    mac_hwapi_set_tx_max_frame_len(rdpa_emac3, p_dpi_cfg->mtu_size);
    mac_hwapi_set_tx_min_pkt_size(rdpa_emac0, MIN_ETH_PKT_SIZE_LAN);
    mac_hwapi_set_tx_min_pkt_size(rdpa_emac1, MIN_ETH_PKT_SIZE_LAN);
    mac_hwapi_set_tx_min_pkt_size(rdpa_emac2, MIN_ETH_PKT_SIZE_LAN);
    mac_hwapi_set_tx_min_pkt_size(rdpa_emac3, MIN_ETH_PKT_SIZE_WAN - 4);
    mac_hwapi_set_backpressure_ext(rdpa_emac0, 1); /* enable backpressure toward SF2 */
    mac_hwapi_set_backpressure_ext(rdpa_emac1, 1); /* enable backpressure toward SF2 */
    mac_hwapi_set_backpressure_ext(rdpa_emac2, 1); /* enable backpressure toward SF2 */
    mac_hwapi_set_rxtx_enable(rdpa_emac0, 1, 1);
    mac_hwapi_set_rxtx_enable(rdpa_emac1, 1, 1);
    mac_hwapi_set_rxtx_enable(rdpa_emac2, 1, 1);
    //mac_hwapi_set_rxtx_enable(rdpa_emac3, 1, 1); /* Ethernet WAN EMAC will be enabled when the WAN service gets created */
}
#endif
