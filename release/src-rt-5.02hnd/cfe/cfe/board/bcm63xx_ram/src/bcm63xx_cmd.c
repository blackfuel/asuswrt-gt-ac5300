...
#include "bcm_ubi.h"
#endif
#if !defined (_BCM96838_) && !defined (_BCM963138_) && !defined (_BCM960333_) && !defined (_BCM963148_) && !defined (_BCM96848_) && !defined(_BCM94908_) && !defined(_BCM96858_) && !defined(_BCM968360_)
...
#elif defined (_BCM963138_) || defined (_BCM963148_) || defined (_BCM96858_) || defined (_BCM94908_) || defined (_BCM968360_)
#if (NONETWORK == 0)
#include "bcm_ethsw.h"
#endif
#include "pmc_drv.h"
...
#include "btrm_if.h"
#if (defined (_BCM96838_) || defined (_BCM96848_) || defined (_BCM96858_)) && (NONETWORK == 0)
...
static char *cras_usage_str = "Usage: cras <ring id> <status>";
#endif
...
static char *lport_stats_usage = "Usage: lport_stats <port id (0-6)> [\"clean\"]";
...
/* FIXMET: enable for 68360 when pmc driver inserted */
...
static int ui_cmd_load_dtb(ui_cmdline_t *cmd,int argc,char *argv[])
{
    ...
    return dtb_prepare(DTB_PREPARE_FDT, ptr, len);
}
#endif
...
static int ui_cmd_write_whole_image(ui_cmdline_t *cmd,int argc,char *argv[])
{
    char hostImageName[BOOT_FILENAME_LEN + BOOT_IP_LEN];
    char *imageName;
#if defined (_BCM963138_) || (_BCM963148_) || defined (_BCM94908_) || defined (_BCM96858_) || defined (_BCM968360_)
    char *pmcName = NULL;
#endif
    uint8_t *ptr = (uint8_t *) cfe_get_mempool_ptr();
    int res;
    ...
    if( (uintptr_t) puiCmpImage > FLASH_BASE )
    {
	    ...
	    pDest = pDestBndry + 1024;
	       ...
	       memcpy((void *)(pDest - SEC_S_SIGNATURE), (void *)(pDest + atoi(pCurTag->kernelLen) - SEC_S_SIGNATURE), SEC_S_SIGNATURE);
            ...
            /* ARM linux kernel compiled for virtual address at 0xc0008000, convert to physical 
               address for CFE */
            pucDst =(unsigned char*)(((uintptr_t)pucDst)&0xfffffff);
            pucEntry = (unsigned char*)(((uintptr_t)pucEntry)&0xfffffff);
#endif
	       ...
	       memcpy((void *)&authArgs, authCreds, sizeof(Booter1AuthArgs));
               ...
               authenticate((uint8_t *)(((uintptr_t)pucSrc) - SEC_S_SIGNATURE - 12), dataLen + 12 + SEC_S_SIGNATURE, authArgs.manu);
               printf("%s\n", "pass"); 
            }
#endif
...
#ifdef USE_LZ4_DECOMPRESSOR
                if( uncomplen ) {
                    ret = LZ4_decompress_fast((const char *)pucSrc, (char *)pucDst, uncomplen) != dataLen;
                } else
#endif
                {
                             ...
                             unsigned int rd_addr)
{
    ...
    if (dtb_prepare(DTB_PREPARE_FDT, ptr, size)) {
        return -1;
    }
    ...
    if (dtb_set_chosen_initrd(rd_addr, size)) {
        return -1; 
    }
...
{
    char brcmMagic[] = {'B','R','C','M'};
    unsigned char *image_buf = NULL, *boot_load_addr, *boot_addr;
    int sector_len = flash_get_sector_size(0), ret = 0;
    unsigned int image_size = 0, sec_image_size = 0; 
    uint32_t uncomplen = 0, data_len = 0, *image = NULL; 

#if defined(_BCM963268_) || defined(_BCM96838_) || defined(_BCM963381_) || \
    defined(_BCM963138_) || defined(_BCM963148_) || \
    defined(_BCM94908_) || defined(_BCM96858_) || defined(_BCM968360_)
     unsigned char* sec_image = NULL;
    if (bcm_otp_is_boot_secure()) { 
        if ( cfe_fs_find_file(NAND_FLASH_BOOT_SIG_NAME, 
                              C_CSTRLEN(NAND_FLASH_BOOT_SIG_NAME), 
                              start_blk, end_blk - start_blk, 
                              &sec_image, &sec_image_size, 0) ) {
             return -1;
        }
...
#error "SEC_S_SIGNATURE must be 4 byte aligned"   
#endif  
        if (SEC_S_SIGNATURE != sec_image_size) {
    ...
    if (
        ...
        cfe_fs_find_file(NAND_FLASH_BOOT_IMAGE_LZ4, C_CSTRLEN(NAND_FLASH_BOOT_IMAGE_LZ4), start_blk, 
                               end_blk - start_blk, &image_buf, &image_size, sec_image_size) && 
#endif
    ...
    defined(_BCM94908_) || defined(_BCM96858_) || defined(_BCM968360_)
    if (sec_image_size ) {
        if (image_buf != sec_image)
            memcpy((void*)image_buf, sec_image, sec_image_size);
        image = (uint32_t*)(image_buf+sec_image_size);
    } else
#endif
    image = (uint32_t*)image_buf;
    boot_load_addr = (unsigned char *) (uintptr_t)(*image);
    boot_addr = (unsigned char *)(uintptr_t)(*(image + 1));

#if defined(CONFIG_ARM) || defined(CONFIG_ARM64)
    /* ARM linux kernel compiled for virtual address at 0xc0008000, 
       convert to physical address for CFE */
    boot_load_addr =(unsigned char*)(((uintptr_t)boot_load_addr)&0xfffffff);
    boot_addr = (unsigned char*)(((uintptr_t)boot_addr)&0xfffffff);
#endif
    /* new image format contains broadcom signature and uncompressed length.*/
    data_len = *(image + 2);
    if ( *(image + 3) == (*(uint32_t*)brcmMagic) ) {
            uncomplen = *(image + 4);
            image += 5;
    } else {
            image += 3;
        ...
        if (bcm_otp_is_boot_secure())
        {
           ...
           if( (CFE_RAM_ROM_PARMS_GET_SEQ_P1 & ~0xffff) == NAND_SEQ_MAGIC &&
              ...
              authenticate((uint8_t *)(((uintptr_t)(unsigned char*)image) - SEC_S_SIGNATURE - sizeof(unsigned char *)*2 - sizeof(uint32_t)), 
                                              data_len + sizeof(unsigned char *)*2 + sizeof(uint32_t) + SEC_S_SIGNATURE, authArgs.manu);
              printf("%s\n", "pass"); 
           }
           else {
...
#ifdef USE_LZ4_DECOMPRESSOR
        if (uncomplen) {           
            ret = LZ4_decompress_fast((const char *)image, (char *)boot_load_addr, uncomplen) != data_len;
        }
        else
#endif
        {
            ret = decompressLZMA((unsigned char*)image, data_len, boot_load_addr, 23*1024*1024) < 0;
        }

    if (ret) {
        printf("Failed to decompress %s image.  ret = %d Corrupted image?\n",uncomplen? "LZ4":"LZMA",ret);
    } else {
        cfe_loadargs_t la;
        memset((unsigned char *) &la, 0x00, sizeof(la));
             ...
             * rootfs to use.
             */
        blparms_set_int(NAND_RFS_OFS_NAME, (start_blk * sector_len) / 1024);
#if !defined(CONFIG_ARM) && !defined(CONFIG_ARM64)
        /* Continue to save the rootfs offset the word before the 
         * destination (load) address for backwards compatibility.
        */
        *(unsigned int *) (boot_load_addr - 4) = (start_blk * sector_len) / 1024;
#endif
        ...
        */
        ...
        printf("Decompression %s Image OK!\n",uncomplen ? "LZ4":"LZMA");
        la.la_address = (long) boot_load_addr;
        la.la_entrypt = (long) boot_addr;
        la.la_flags = LA_DEFAULT_FLAGS;
        printf("Entry at 0x%p\n",la.la_entrypt);
        cfe_go(&la);  // never return...
	/* if we returne by any chance free uneeded memory*/
    }
    return( ret );
}
...
static int bootNandImage(void)
{
    ...
    if( pTag1 && pTag2 ) {
        ...
        if( bootInfo.bootPartition == BOOT_SET_NEW_IMAGE )
        {
...
static int ui_cmd_force_spi_nand(ui_cmdline_t *cmd, int argc, char *argv[])
{
    ...
    flash_init(); // do flash init again, but with g_foce_mode on will now default to SPI NAND
...
{
#if !defined (_BCM96838_) && !defined (_BCM960333_) && !defined (_BCM96848_) && !defined (_BCM947189_)
    if (argc < 2) {
...
{
#if !defined (_BCM96838_) && !defined (_BCM963138_) && !defined (_BCM960333_) && !defined (_BCM963148_) && !defined (_BCM96848_) && !defined(_BCM94908_) && !defined(_BCM96858_) && !defined(_BCM947189_) && !defined(_BCM968360_)
    uint8 data[8];
    char *pszAddr = cmd_getarg(cmd, 0);
...
{
    char *pszMargin1 = cmd_getarg(cmd, 0);
    char *pszMargin2 = cmd_getarg(cmd, 1);
    int res;
    uint32_t val1 = (uint32_t)atoi(pszMargin1);
    uint32_t val2  = (uint32_t)atoi(pszMargin2);

    res = CloseAVS(val1, val2);
    if( res == 0 )
...
static int ui_cmd_runner_admin_ring(ui_cmdline_t *cmd, int argc, char *argv[])
{
    uint32_t ring_id, status;
    char *str1, *str2;
    ...
    if (ring_id >= D_NUM_OF_RING_DESCRIPTORS || status > 1)
    {
        xprintf("Wrong params values\nMax ring index %d; status: <0|1>\n", D_NUM_OF_RING_DESCRIPTORS);
        return -1;
    }

    return cpu_ring_shell_admin_ring(NULL, ring_id, status);
}
#endif
...
static int ui_cmd_lport_stats(ui_cmdline_t *cmd, int argc, char *argv[])
{
    ...
    if ((port = lport_port_validate(cmd, lport_stats_usage, 0, 6)) == -1)
        return -1;
...
#if (defined(_BCM94908_) || defined(_BCM96858_)) && (INC_NAND_FLASH_DRIVER==1)
...
static int ui_init_bcm63xx_cmds(void)
{
    ...
    cmd_addcmd("loadb",
               ...
               "loadb op [address] [[hostip:]filename]: op n for network, s for serial port. if no address - load to predefined address \n\n"
               "",
               "");
  ...
  cmd_addcmd("cras",
               ui_cmd_runner_admin_ring, 
               NULL,
               "Configure ring admin status",
               cras_usage_str,
               "");
#endif

#if defined(_BCM96858_) && (NONETWORK == 0)
  cmd_addcmd("lport_rate",
               ui_cmd_lport_rate,
               NULL,
...
/* Exception case handling to detect A0 image on B0 and vice versa */
    ...
    !defined(_BCM96858_) && !defined(_BCM968360_) && !defined(_BCM947189_)
    bcm_ethsw_init();
#endif
