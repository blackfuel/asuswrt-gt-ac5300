...
wlc_valid_chanspec_ext(wlc_cm_info_t *wlc_cmi, chanspec_t chspec, bool dualband)
{
	...
	if (CHSPEC_IS5G(chspec) &&
		(wlc->disband5grp & (1 << CHANNEL_POWER_IDX_5G(channel)))) {
		return FALSE;
	}
...
static char *
clm_data_ver2str(unsigned int vms)
{
	static char verstr[100];

	sprintf(verstr, "%d.%d", (vms>>16)&0xFFFF, vms&0xFFFF);

	return verstr;
}

extern const struct clm_data_version clm_version;

void
wlc_dump_clm_data_ver(struct bcmstrbuf *b)
{
	bcm_bprintf(b, "%s %s\n", clm_version.productid, clm_data_ver2str(((clm_version.major_version) << 16) | clm_version.minor_version));
}
