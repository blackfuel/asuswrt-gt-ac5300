...
pciedev_schedule_flow_ring_read_buffer(struct dngl_bus *pciedev)
{
	...
	while (!dll_end(prio, &pciedev->active_prioring_list)) {
		...
		while (!dll_end(cur, &prioring->active_flowring_list)) {
			...
			if (FFSHCED_ST_ENABLED(pciedev)) {
				if (flow_ring->status & FLOW_RING_PKT_RESET) {
					flow_ring->activetimer = hnd_time();
				}
...
/**
 * Calculating the max pkt fetch count budget to the flowring based
 * in the weight.
 */
static inline void
pciedev_flow_ring_calc_maxpktcnt(struct dngl_bus *pciedev,
	msgbuf_ring_t *flow_ring)
{
	ASSERT(SCHEDCXT_FL_W(flow_ring));
	ASSERT(SCHEDCXT_SUM_W(pciedev));
	...
	while (!dll_end(prio, &pciedev->active_prioring_list) && do_fetch) {
		...
		while (do_fetch) {
			...
			if (FFSHCED_ST_ENABLED(pciedev) && SCHEDCXT_FL_W(flow_ring)) {
				...
				pciedev_flow_ring_calc_maxpktcnt(pciedev, flow_ring);
			}
	...
	if (FFSHCED_ST_ENABLED(pciedev) && SCHEDCXT_FL_W(flow_ring)) {
		...
		if (tot_pkts < SCHEDCXT_FL_MAXLFRAGS(flow_ring)) {
			...
			if (SCHEDCXT_SUM_W(pciedev) > SCHEDCXT_FL_W(flow_ring)) {
				...
				* l = L x w/W;
				*/
				ret_len = (lbuf_avail * SCHEDCXT_FL_W(flow_ring)) /
					SCHEDCXT_SUM_W(pciedev);
	...
	while (!dll_end(prio, &pciedev->active_prioring_list)) {
		...
		while (!dll_end(cur, &prioring->active_flowring_list)) {
			...
			if (params != NULL) {
				...
				if (!(flow_ring->status & FLOW_RING_PKT_RESET)) {
					uint32 now = hnd_time();
					/* Donot lfrag_max based on txpkts in the first 3sec,
					 * due to traffic slow starts
					 */
					if ((flow_ring->status & FLOW_RING_PKT_RESETING) ||
					   ((flow_ring->activetimer != 0) &&
					   (now - flow_ring->activetimer >= FFSCHED_FLOW_RING_ACTIVE_DELAY))) {
						 ...
						 */
						if (pciedev->flr_lfrag_txpkts_adjust && !(flow_ring->mumimo)) {
							flow_ring->lfrag_max /=
...
void pciedev_set_ffsched(struct dngl_bus * pciedev, void *params)
{
	if (params != NULL) {
		...
		if ((on ? TRUE : FALSE) == FFSHCED_ENAB(pciedev))
			return;
	...
	msgbuf_ring_t *flow_ring)
{
	...
	if (flow_ring->inited) {
		flow_ring->status |= FLOW_RING_PKT_RESETING;
...
static void pciedev_update_rdp_ptr_unacked(msgbuf_ring_t *flow_ring)
{
	uint16 tmp_len, k, rdpt, start_rd_pending, max_item;

	start_rd_pending = flow_ring->rd_pending;
	if (!isset(flow_ring->status_cmpl, MODULO_RING_IDX(start_rd_pending, flow_ring)))
		return;

	/* The next part was (re-)written in a way that it is more efficient to compile. Less
	 * instructions and it avoids a jump (pipeline flush). The downside is that it can go
	 * through 0, and this may lead to coverity complaints. It is also harder to read; But the
	 * idea is to calculate the number of indexes from rd_pending till w_ptr taking wrap around
	 * in account.
	 */
	max_item = RING_MAX_ITEM(flow_ring);
	tmp_len = DNGL_RING_WPTR(flow_ring) - start_rd_pending;
	if (DNGL_RING_WPTR(flow_ring) < start_rd_pending)
		tmp_len += max_item;

	for (k = 0; (k < tmp_len) && (k < flow_ring->bitmap_size - 1); k++) {
		rdpt = (start_rd_pending + k) % max_item;
		if (isset(flow_ring->inprocess, MODULO_RING_IDX(rdpt, flow_ring))) {
			if (isset(flow_ring->status_cmpl, MODULO_RING_IDX(rdpt, flow_ring))) {
				flow_ring->rd_pending = (flow_ring->rd_pending + 1) % max_item;
			} else {
				flow_ring->rd_pending = rdpt;
				break;
			}
		} else {
			/* Break out as soon as we hit a non in-process element */
			break;
		}
	}
}
...
pciedev_resource_avail_for_txmetadata(struct dngl_bus *pciedev)
{
	...
	if (!CIR_BUFPOOL_AVAILABLE(cpool)) {
