...
int __init bcmusbnic_probe( void )
{
...
#endif
	{
        ...
        if( nStatus == 0 )
        {
            ...
            /* This is needed because USB host controller is not suppressed when in device mode on 6318 */
            printk(" Disabling USB EHCI and OHCI interrupts \n");
            /* Disable interrupts. First disable host controller interrupts */
            BcmHalInterruptDisable(INTERRUPT_ID_USBH);
            BcmHalInterruptDisable(INTERRUPT_ID_USBH20);
#endif            
...
LOCAL int UsbNicInit( PUSBNIC_DEV_CONTEXT pDevCtx, int initFlag )
{
   ...
   GPIO->PinMuxSelOther &= ~PINMUX_SEL_USB_MASK;
   GPIO->PinMuxSelOther |= PINMUX_SEL_USB_DEV;
#endif
   USBH->SwapControl |= USB_DEVICE_SEL ;

#if defined(CONFIG_BCM96318)
   /* The below sequence of configuration is important. Do not rearrange them */
   USBH->UTMIControl1 &= ~(UTMI_PHY_MODE_MASK);
   USBH->UTMIControl1 |= UTMI_PHY_MODE_DEVICE;    

   GPIO->PinMuxSel2 &= ~(PINMUX_SEL_GPIO40_MASK << PINMUX_SEL_GPIO40_SHIFT );
   GPIO->PinMuxSel2 |= (PINMUX_SEL_USB_ACTIVE << PINMUX_SEL_GPIO40_SHIFT);
   
   GPIO->PinMuxSel0 &= ~(PINMUX_SEL_GPIO13_MASK << PINMUX_SEL_GPIO13_SHIFT );
   GPIO->PinMuxSel0 |= (PINMUX_SEL_USB_DEVICE_LED << PINMUX_SEL_GPIO13_SHIFT);   
#endif
   ...
   if (nStatus == 0) {
            ...
            i < NR_CTRL_BDS; i++, pBd++, pCtrlPkt++ )
      {
         ...
         pBd->address = (UINT32) VIRT_TO_PHY(pCtrlPkt);
         pCtrlPkt->pBd = pBd;
      }
      ...
      pDevCtx->pDmaRegs->stram.s[USB_IRQ_TX_CHAN].baseDescPtr =
         (UINT32) VIRT_TO_PHY(pDevCtx->pTxIntrBdBase);
    ...
    PUSBNIC_DEV_CONTEXT pDevCtx)
{
    ...
    if( pTxHdr )
    {
        ...
        pBd->address = (UINT32) VIRT_TO_PHY(pData + ulAlignLen);
...
LOCAL void usbnic_assign_buffer(UINT8 * pBuf, PUSBNIC_DEV_CONTEXT pDevCtx)
{
    ...
    {
        pAssign->address = VIRT_TO_PHY((unsigned long) pBuf);
    ...
    UINT32 ulLength, UINT8 advanceCurr)
{
	...
	if( (pTxDma->cfg & DMA_ENABLE) == 0 )
	{
      ...
      DUMPADDR(pBuf, ulLength);
		...
		// be completed.
	   pBd->address = (UINT32) VIRT_TO_PHY(pBuf);	// just in case alignment matters here, too
    ...
    UINT32 ulLength)
{
	...
	if( (pTxDma->cfg & DMA_ENABLE) == 0 )
	{
      ...
      DUMPADDR(pBuf, ulLength);
		...
		// be completed.
		if (ulLength == 0) {
			pBd->address = (UINT32) VIRT_TO_PHY(&nullMsg);
			pBd->length = 1 ;
