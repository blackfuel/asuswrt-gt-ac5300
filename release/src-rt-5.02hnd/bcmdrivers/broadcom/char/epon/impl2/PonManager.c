...
////////////////////////////////////////////////////////////////////////////////
//extern
void PonMgrSetDefaultFec(LinkIndex link)
    {
    if(PonCfgDbGetUpRate() ==  PonCfgDbGetDnRate())
        {
        if(PonCfgDbGetUpRate() == LaserRate1G)
            {
             // ensure that 1/1G FEC and encryption are disabled
            (void)FecModeSet(link, FALSE, FALSE, FecApp1G);
            }
        else
            {
            (void)FecModeSet(link, true, true, FecApp10G);
            }
        }
    else
        {
        (void)FecModeSet(link, true, FALSE, FecAppBoth);
        }
    }
...
void PonMgrEponMacQueueConfigInit(epon_mac_q_cfg_t *cfg)
    ...
    switch (oam_mode)
        ...
        case rdpa_epon_bcm:
            ...
            if (PonCfgDbGetUpRate() == LaserRate10G)
                {
                cfg->link_num = BcmRpt10gMaxLinkNum-TkOnuRsvNumRxOnlyLlids;
                }
            else
                {
                cfg->link_num = BcmRpt1gMaxLinkNum-TkOnuRsvNumRxOnlyLlids;
                }
