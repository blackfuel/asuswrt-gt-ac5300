...
UINT32 XTM_ASM_HANDLER::UpdateResetFsm (UINT32 ulLogicalPort, int wrongCheck, int newMsgType)
{
...
#if defined(ASM_GFC_IGNORE)
			 i |= TXSARA_SID12_EN ;
#else
			 i &= ~TXSARA_SID12_EN ;  /* Even in 12-bit mode, Tx should be 8-bit for 6368 to avoid
                                      Non-Zero GFC in ASMs.
                                    */
#endif
			 j |= RXSARA_SID12_EN ;
		 }
...
int XTM_ASM_HANDLER::ProcessReceivedAsm (Asm *pAsm, UINT32 ulLogicalPort)
{
...

   {
      ...
      newResetFsmState = UpdateResetFsm (ulLogicalPort, checkFailed, pAsm->msgType) ;
	...
	if( !m_ulRegistered )
	{
      ...
      m_BondMgmt.asmCell.rxAsmStatus[0] = 0xC0 ;
		...
		bondMgmtValid = 1 ;
                ...
                XtmOsPrintf ("bcmxtmcfg: ATM Bonding Mgmt Log Area = %x \n", (unsigned int) &m_BondMgmt.printAsm) ;
	}
	else
