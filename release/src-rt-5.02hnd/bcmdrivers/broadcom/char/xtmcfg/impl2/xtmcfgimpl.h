...
public:
    ...
    BCMXTM_STATUS SetLinkInfo( UINT32 ulLinkMask, PXTM_INTERFACE_LINK_INFO pLinkInfo,
                               UINT32 ulBondingPort, PXTM_INTERFACE_LINK_INFO pOtherLinkInfo, UINT32 ulxDSLGfastMode);
 ...
 */
#define XTM_MAJ_VER           12
#define XTM_MIN_VER           3
#define XTM_BUILD_VER         3
...
public:
    ...
    int LineIdle(unsigned long);     // Line has entered idle state.
    int LineActive(unsigned long);   // Line has entered nonidle (active or training) state.
...
private:
    ...
    static void XtmMonitorTimerCb (UINT32 ulParm) ;
