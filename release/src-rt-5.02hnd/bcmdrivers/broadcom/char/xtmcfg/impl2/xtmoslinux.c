...
UINT32 XtmOsStartTimer( void *pFnEntry, UINT32 ulFnParm, UINT32 ulTimeout )
{
    ...
    /* Loop through timers to find a free one to use */
    for( i = 0, pTimer = &g_pData->IntDataTimer[0]; i < MAX_INT_DATA_TIMERS;
        i++, pTimer++ )
    {
        if ((g_pData->IntDataTimerHdlr[i] == NULL) ||
            ((g_pData->IntDataTimerHdlr[i] == pFnEntry) && !g_pData->IntDataTimerShutDown[i]))
        {
        ...
        i++, pTimer++ )
    {
        ...
        if( timer_pending( pTimer ) )
        {
            ...
            if(pTimer->function == pFnEntry) {
                ...
                if(iRet == -1) {
                    iRet = 1;  /* One timer canceled, so hard code to 1 */
                } else {
                    iRet++; /* Increment tiemr canceled */
                }
            }
        }
    }
