...
#define pHostGso(_fapIdx) ( &pHostPsmGbl(_fapIdx)->gso )
#else
...
void fapIq_dumpStatus( void );
void fapIq_setStatus( int status );
void fapIq_add_L4port( uint8_t ipProto, uint16_t dport, 
            uint8_t ent, uint8_t prio );
void fapIq_rem_L4port( uint8_t ipProto, uint16_t dport, uint8_t ent );
void fapIq_dump_porttbl( uint8_t ipProto );
#endif
...
#if defined(CC_FAP4KE_PMON)
#if defined(CONFIG_BCM96362)
#define pHostPmon(_fapIdx) ( &pHostPsmGbl(_fapIdx)->pmon )
#else
...
int __init fapDrv_construct(void)
{
    ...
    if((chipId & 0xFF) == 0xB0)
    {
        printk("********** ERROR: Running FAP A0 code in B0 chip **********\n");
        while(1);
    }
#else
    if((chipId & 0xFF) == 0xA0)
    {
        printk("********** ERROR: Running FAP B0 code in A0 chip **********\n");
        while(1);
    }
#endif
    ...
    gbpm_fap_status_hook_g = fapBpm_dumpStatus;
    gbpm_fap_thresh_hook_g = fapBpm_dumpTxQThresh;
    gbpm_fap_enet_thresh_hook_g = fapBpm_dumpEthTxQThresh;
    gbpm_fap_upd_buf_lvl_hook_g = fapBpm_updBufLvl;
    ...
    iqos_fap_status_hook_g = fapIq_dumpStatus;
    iqos_fap_set_status_hook_g = fapIq_setStatus;
    iqos_fap_add_L4port_hook_g = fapIq_add_L4port;
    iqos_fap_rem_L4port_hook_g = fapIq_rem_L4port;
    iqos_fap_dump_porttbl_hook_g = fapIq_dump_porttbl;
...
#if (defined(CONFIG_BCM_BPM) || defined(CONFIG_BCM_BPM_MODULE))
    /* unbind BPM hooks */
    gbpm_fap_evt_hook_g = NULL;
    gbpm_fap_status_hook_g = NULL;
    gbpm_fap_thresh_hook_g = NULL;
    gbpm_fap_enet_thresh_hook_g = NULL;
    gbpm_fap_upd_buf_lvl_hook_g = NULL;
#endif

#if (defined(CONFIG_BCM_INGQOS) || defined(CONFIG_BCM_INGQOS_MODULE))
    /* unbind Ingress QoS hooks */
    iqos_fap_status_hook_g = NULL;
    iqos_fap_add_L4port_hook_g = NULL;
    iqos_fap_rem_L4port_hook_g = NULL;
    iqos_fap_dump_porttbl_hook_g = NULL;
...
int fapBpm_pendEvtHandle( DQMQueueDataReg_S *msg_p )
{
    ...
    switch (cmd)
    {
        case HOSTMSG_CMD_ALLOC_BUF_REQT:
        {
            ...
            {
                *memPtr =
                    (void *) PFKBUFF_TO_PDATA((*pBuf),BCM_PKT_HEADROOM);
            }
        ...
        case HOSTMSG_CMD_FREE_BUF_REQT:
        {
            ...
            /*
             * pData to pFKB conversion
             * BPM stores address of pFKB, whereas FAP used pData,
             * so before freeing of buffers to BPM, pData needs to be
             * converted to pFKB
             * TODO: optimization
             */
            memPtr = mappedMemAddr;
            ...
            {
                *memPtr =
                    (void *) PDATA_TO_PFKBUFF((*memPtr),BCM_PKT_HEADROOM);
            }
