                               ...
                               fap4kePkt_flowInfo_t *flowInfo_p)
{
    ...
    if( blog_p->insert_eth )
    {
        ...
        /* in PPPoA encapsulation there is no Ethernet header, hence no VLAN headers */
    ...
    else
    {
        if(isSrcExtSWPort)
        {
#if defined(CONFIG_BCM96362) && defined(CONFIG_BCM_PORTS_ON_INT_EXT_SW)
           learnAction_p->cmd[learnAction_p->cmdCount++] = FAP4KE_PKT_CMD_POP_BRCM_TAG;;
#endif
        ...
        /*  Pop the BRCM Tag for 6362 and 6818. No need to pop the BRCM tag for 63268 and later chips */
        else if ((flowInfo_p->source.phy == FAP4KE_PKT_PHY_ENET)
        )
        {
            learnAction_p->cmd[learnAction_p->cmdCount++] = FAP4KE_PKT_CMD_POP_BRCM_TAG;
        }
            ...
            if(lA1_p->cmd[i] == lA2_p->cmd[i])
            {
                switch (lA1_p->cmd[i])
                {
                    ...
                    case FAP4KE_PKT_CMD_COPY_VLAN_HDR:
                        ...
                        vlanHdr2_p = &lA1_p->cmdArg.vlanHdr[numVlans];
                        numVlans++;
                        break;
                            ...
                            fap4kePkt_flowInfo_t *flowInfo_p)
{
    ...
    mcCfglog_addTxDev(mcCfglog_p,blog_p->tx.dev_p);
    ...
    flowInfo_p->ipv4.tuple.excludeDestPort = blog_p->rx.tuple.port.dest;
                      ...
                      int *isActivation_p)
{
    ...
    *isActivation_p = 0;
    ...
    if(blog_p->mc_sync)
    {
        fapMcast_info("SYNC :dev=%pS >>>>", blog_p->tx.dev_p);
    ...
    else if(mcastFlow_p == NULL)
    {
        *isActivation_p = 1;
        ...
        fapMcast_info("CREATE :dev=%pS >>>>", blog_p->tx.dev_p);
    ...
    else /* mcast flow already exists */
    {
        ...
        fapMcast_info("ADD DEST PORT :dev=%pS >>>>", blog_p->tx.dev_p);
        ...
        while(mcCfglog_p)
        {
            if(mcCfglog_findTxDev(mcCfglog_p, blog_p->tx.dev_p) >=0)
            {
                /* destination port already exists - return index */
                fapMcast_info("Destination dev %pS already exists", blog_p->tx.dev_p);
        ...
        if(!fapMcast.useIudma2Bd)
        {
            ...
            if(mcCfglog_p)
            {
                ...
                if(mcCfglog_addTxDev(mcCfglog_p,blog_p->tx.dev_p) < 0)
                {
                    ret = FAP_ERROR;
                    goto out;
                } 
                ...
                if (ret == FAP_ERROR)
                {
                    mcCfglog_delTxDev(mcCfglog_p,blog_p->tx.dev_p);
...
int fapMcast_deactivate(Blog_t *blog_p, int *isDeactivation_p)
{
    ...
    *isDeactivation_p = 0;
    ...
    {
        fapMcast_info("REM DESTPORT: dev=%pS >>>>", blog_p->tx.dev_p);
        mcastFlow_p->nClients--;
        nClients = mcastFlow_p->nClients;

        /* Remove dest port from FAP flow associated with the Multicast Flow */
        if(mcCfglog_delTxDev(mcCfglog_p, blog_p->tx.dev_p) < 0)
        {
            ret = FAP_ERROR;
            goto out;
        }
