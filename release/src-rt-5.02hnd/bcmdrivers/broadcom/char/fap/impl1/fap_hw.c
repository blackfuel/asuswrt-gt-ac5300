...
void fapHostHw_PrintRegs(uint32 fapIdx, fapRegGroups_t regGroup)
{
  ...
  if (regGroup==FAP_CNTRL_REG || regGroup==FAP_ALL_REG)
  {
    ...
    FAP_PRINT_REG(hostRegCntrl(fapIdx), dma4_7_fifo_status);
    FAP_PRINT_REG(hostRegCntrl(fapIdx), dma_irq_sts);
    FAP_PRINT_REG(hostRegCntrl(fapIdx), dma_4ke_irq_mask);
    FAP_PRINT_REG(hostRegCntrl(fapIdx), dma_host_irq_mask);
#endif
...
#if defined(CONFIG_BCM96362)
    FAP_PRINT_REG(hostRegCntrl(fapIdx), mbist_tm);
