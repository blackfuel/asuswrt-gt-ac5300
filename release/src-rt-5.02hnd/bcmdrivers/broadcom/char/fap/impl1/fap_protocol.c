...
/* This is for FHW layer */
uint16_t fapGetEntIx(uint32_t hwTuple)
{
    return ( ((FAP_MAX_FLOWS * NUM_FAPS) * fapGetHwEngine(hwTuple)) +
        fapGetHwEntIx(hwTuple));
}
...
static void buildClog(Blog_t *blog_p, Clog_t *clog_p)
{
    ...
    if(blog_p->rx.info.bmap.BCM_SWC)
    {
        clog_p->rx_l2hdr_p = &clog_p->rx_l2hdr[0];
        clog_p->rx_length = blog_p->rx.length - BRCM_TAG_LENGTH;

        memcpy(&clog_p->rx_l2hdr[0], &blog_p->rx.l2hdr[0], (2*ETH_ALEN));
        memcpy(&clog_p->rx_l2hdr[(2*ETH_ALEN)],
                &blog_p->rx.l2hdr[(2*ETH_ALEN) + BRCM_TAG_LENGTH],
                clog_p->rx_length - (2*ETH_ALEN));
    }
    else    /* No BRCM_TAG present */
#endif
    {
...
abort_activate:
    ...
    return FAP_HW_TUPLE(FAP_HW_ENGINE_SWC, FAP4KE_PKT_INVALID_FLOWHANDLE);
}
...
void fapStatus(void)
{
    ...
    protoPrint("FAP:\n"
               ...
               fapState_g.active, NUM_FAPS*FAP4KE_PKT_MAX_FLOWS,
#if defined(CC_FAP4KE_PKT_IPV6_SUPPORT)
               "Enabled",
#else
               "Disabled",
...
void fapEnable(void)
{
    ...
    hwHooks.activate_fn = (HOOKP)fapActivate;
    ...
    hwHooks.get_hw_entix_fn = (HOOK32)fapGetEntIx; 
