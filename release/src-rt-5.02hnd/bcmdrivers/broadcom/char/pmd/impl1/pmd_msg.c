...
void msg_system_init(void)
{
    ...
    msg_seq_num[hmid_software_version_get] = 1;
    ...
    msg_seq_num[hmid_cal_result_get] = 23;
    msg_seq_num[hmid_bias_cur_get] = 1;
    msg_seq_num[hmid_level1_tracking_get] = 1;
    ...
    msg_seq_num[hmid_apd_config_set] = 1;
    ...
    msg_seq_num[hmid_cal_param_set] = 6;
...
static int set_ingress_mailbox(hm_message *msg)
{
    ...
    BCM_LOG_DEBUG(BCM_LOG_ID_PMD, "mailbox reg = %x (sync %d status %d seq %d id %x value %x). error = %d\n",
...
#ifdef PMD_FULL_DEBUG
    BCM_LOG_DEBUG(BCM_LOG_ID_PMD, "mailbox = %x (sync %d status %d seq %d id %x value %x). error = %d\n", mailbox, msg->sync, msg->status, msg->seq, msg->id, msg->value, rc);
#endif
    return 0;
}
...
int pmd_msg_handler(hm_msg_id id, int16_t *buf, uint16_t len)
{
...
#ifdef PMD_FULL_DEBUG
    BCM_LOG_DEBUG(BCM_LOG_ID_PMD, ",msg id %x msg transactions %d sync %d  \n", (int)id, (int)msg_seq, sync_bit );
#endif
    if (id > hmid_last || msg_seq == 0)
    {
        BCM_LOG_ERROR(BCM_LOG_ID_PMD, ",msg id %x not defined \n", (int)id);
        return -1;
    }

    if (len != (msg_seq * 2))
    {
        BCM_LOG_ERROR(BCM_LOG_ID_PMD, ",Wrong len: msg %x expecting %d \n",(int)id, (msg_seq *2));
        return -1;
    }
    ...
    while (msg_state != finish_msg_op)
    {
        ...
        if (rc)
        {
            BCM_LOG_ERROR(BCM_LOG_ID_PMD, "Failed in msg %x state %d \n",id, msg_state);
            break;
        }
    }
