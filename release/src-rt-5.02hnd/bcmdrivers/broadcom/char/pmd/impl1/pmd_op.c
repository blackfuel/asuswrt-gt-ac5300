...
#define PMD_REG_FILE_NAME       "/data/pmd_reg" /*keeps the register mapping */
#define PMD_CAL_FILE_NAME       "/data/pmd_calibration" /*keeps the pmd calibration values */
#define PMD_DUMP_DATA_NAME       "/data/pmd_dump_data" /*keeps the pmd dump data*/
#define PMD_FW_IMG_FILE_NAME    "/etc/pmd_fw_img.bin"
#define PMD_TEMP2APD_FILE_NAME  "/data/pmd_temp2apd"
#define SW_DOWNLOAD_CHUNK       16
#define MAX_READ_SIZE           24
#define MIN_TRANSACTION_SIZE    4

#define PMD_CLIENT_NUM 4
const char* client_string[PMD_CLIENT_NUM] = {"REG", "IRAM", "DRAM", "CAL"};
...
int pmd_op_i2c(pmd_dev_client client, uint16_t offset, unsigned char *buf, uint16_t len, int read_op)
{
    ...
    if (read_op)
    {
        ...
        for (i = 0; i < num_of_transaction ; i++)
        {
            ...
            rc = gponPhy_read(client, i2c_buf, length_with_redundancy);
    ...
    else
    {
        ...
        rc = gponPhy_write(client, i2c_buf, length_with_redundancy) ;
        if(rc != length_with_redundancy)
            error_flag = 1;
    }
    kfree(i2c_buf);
    if (error_flag)
    {
        BCM_LOG_ERROR(BCM_LOG_ID_PMD, "Error in %s operation to PMD\n", (read_op ? "read" : "write"));
        return -1;
    }
    return 0;
}
...
int pmd_op_file(pmd_dev_client client, unsigned char * buf, int len, bool read_op)
{
    ...
    {
    case pmd_reg_map:
        fp = filp_open(PMD_REG_FILE_NAME, flags, mode);
        break;
    case pmd_iram_map:
        fp = filp_open(PMD_FW_IMG_FILE_NAME, flags, mode);
        break;
    case pmd_cal_map:
        fp = filp_open(PMD_CAL_FILE_NAME, flags, mode);
        break;
    case pmd_temp2apd_map:
        fp = filp_open(PMD_TEMP2APD_FILE_NAME, flags, mode);
        break;
    case pmd_dump_data_map:
        fp = filp_open(PMD_DUMP_DATA_NAME, flags, mode);
        break;
    default:
    ...
    if (IS_ERR(fp))
    {
        BCM_LOG_DEBUG(BCM_LOG_ID_PMD, "Failed to open %s file\n", client_string[client]);
        return -1;
    }
    ...
    if (read_op)
    {
        ...
        if (fp->f_op && fp->f_op->read)
        {
            ...
            if ((rc = (int)fp->f_op->read(fp, (void *) buf, len, &fp->f_pos)) <= 0)
                BCM_LOG_DEBUG(BCM_LOG_ID_PMD, "Failed to read from %s\n", client_string[client]);
        }
    }
    else
    {
        if (fp->f_op && fp->f_op->write)
        {
            ...
            if ((rc = (int) fp->f_op->write(fp, (void *) buf, len, &fp->f_pos)) == len)
            {
                ...
                BCM_LOG_DEBUG(BCM_LOG_ID_PMD, "Failed to write to %s\n", client_string[client]);
        }
    }
    ...
    if (rc != len)
    {
        BCM_LOG_DEBUG(BCM_LOG_ID_PMD, "%s operation performed on only %d out of %d asked \n",
...
int pmd_op_get_file_len(pmd_dev_client client, uint16_t * len)
{
    ...
    {
    case pmd_reg_map:
        file = PMD_REG_FILE_NAME;
        break;
    case pmd_iram_map:
        file = PMD_FW_IMG_FILE_NAME;
        break;
    case pmd_cal_map:
        file = PMD_CAL_FILE_NAME;
        break;
    case pmd_temp2apd_map:
        file = PMD_TEMP2APD_FILE_NAME;
        break;
    default:
        BCM_LOG_DEBUG(BCM_LOG_ID_PMD, "client is out of range \n");
        return -1;
...
int pmd_op_temp_conv_table_download(uint32_t *ptr, int elem, int lowest)
{
    int rc;
    int i;
    uint32_t cksum = 0;
    ...
    cksum += file_checksum(((char *)(ptr)), sizeof(uint32_t)* elem);
    ...
    if (fwcksum != cksum)
    {
        BCM_LOG_ERROR(BCM_LOG_ID_PMD, " Temperature table checksum error \n");
        rc = -1;
    }
...
int pmd_op_temp_apd_conv_table_download(uint16_t *ptr, int elem, int lowest)
{
    int rc;
    int i;
    uint32_t tmp = 0;
    ...
    tmp = lowest;
    rc = pmd_op_i2c(pmd_dram_map, APD_TEMP_TABLE_ADDR + sizeof(uint32_t), ((char *)&tmp), sizeof(uint32_t), PMD_WRITE_OP);
    if (rc)
        goto exit;

    tmp = 1;
    rc = pmd_op_i2c(pmd_dram_map, APD_TEMP_TABLE_ADDR + (sizeof(uint32_t) * 2), ((char *)&tmp), sizeof(uint32_t), PMD_WRITE_OP);
    if (rc)
        goto exit;
...
int pmd_op_sw_download(void)
{
    ...
    for (i = 0; i < loops + 1; i++)
    {
        ...
        cksum = file_checksum(file + chunk_offset, chunk);

        rc = pmd_op_i2c(pmd_iram_map, chunk_offset, file + chunk_offset, chunk, PMD_WRITE_OP);
        if (rc)
            goto exit;

        /* check file was downloaded correctly */
        rc = pmd_op_i2c(pmd_iram_map, chunk_offset, file + chunk_offset, chunk, PMD_READ_OP);
        if (rc)
            goto exit;

        if (cksum != file_checksum(file + chunk_offset, chunk))
        {
            BCM_LOG_ERROR(BCM_LOG_ID_PMD, "SW download failed \n");
            rc = -1;
            goto exit;
        }
    }
...
int pmd_temp2apd_file_op(uint16_t* p_pmd_temp_apd_conv)
{
    int rc = 0;
    uint16_t len;
    uint16_t pmd_temp_apd_conv[APD_TEMP_TABLE_SIZE];

    pmd_op_get_file_len(pmd_temp2apd_map, &len);

    if (p_pmd_temp_apd_conv == NULL)
    {
        /*create the temp2apd file if not already created*/
        if (pmd_op_file(pmd_temp2apd_map, (unsigned char *)pmd_temp_apd_conv, len, PMD_READ_OP))
        {
            memset(pmd_temp_apd_conv, 0, sizeof(pmd_temp_apd_conv));
            /* create new temp2apd file */
            pmd_op_file(pmd_temp2apd_map, (unsigned char *)pmd_temp_apd_conv, sizeof(pmd_temp_apd_conv), PMD_WRITE_OP);
        }

        /* downlad table to PMD */
        pmd_op_temp_apd_conv_table_download(&pmd_temp_apd_conv[0], APD_TEMP_TABLE_SIZE, APD_TEMP_TABLE_LOWEST_TEMP);
    }
    else
    {
        if (len != sizeof(pmd_temp_apd_conv))
        {
            BCM_LOG_ERROR(BCM_LOG_ID_PMD, "temp to apd file length %d != %d \n", len, (uint32_t)(sizeof(pmd_temp_apd_conv)));
        }
        pmd_op_file(pmd_temp2apd_map, (unsigned char *)p_pmd_temp_apd_conv, sizeof(pmd_temp_apd_conv), PMD_WRITE_OP);

        /* downlad table to PMD */
        pmd_op_temp_apd_conv_table_download(p_pmd_temp_apd_conv, APD_TEMP_TABLE_SIZE, APD_TEMP_TABLE_LOWEST_TEMP);
    }
