...
static pmd_data_collection pmd_data_col;
...
void pmd_data_col_print(void)
{
    ...
    printk("bias_min    bias_max    mod_min    mod_max    bias_avg    mod_avg      offset0      offset1      ext_temp      die_temp \n");

    index = pmd_data_col.index;

    for (i = 0; i < DATA_COL_SIZE; i++)
    {
        if (index < DATA_COL_SIZE)
        {
...
int pmd_data_collect(void)
{
    int rc;
    int16_t val[pmd_max_col];
