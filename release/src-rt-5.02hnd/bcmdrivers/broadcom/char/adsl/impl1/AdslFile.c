...
extern void * AdslCoreSetSdramImageAddr(ulong lmem2, ulong pgSize, ulong sdramSize);
...
int AdslFileLoadImage(char * fname, void *pAdslLMem, void *pAdslSDRAM)
{
	...
	pAdslSDRAM = AdslCoreSetSdramImageAddr(ADSL_ENDIAN_CONV_LONG(lmemhdr[2]), ADSL_ENDIAN_CONV_LONG(lmemhdr[3]), phyHdr.sdramSize);
#else
