...
Private void CreateVectorReadCmdSeq(void *gDslVars,VDSLPMDVectorReadCmdToneGrp* CmdGrp, int MaxGrpSize,bandPlanDescriptor *bandplan, int gFactor)
{
	...
	do{
		...
		__SoftDslPrintf(gDslVars,"bpGrpNum %d(%d,%d) cmdGrpNum %d StartGp=%d StopGp=%d",0,bp,bpStartTone,bpEndTone,n,CmdGrp->toneGroups[n].startToneGp,CmdGrp->toneGroups[n].stopToneGp);
#endif
		n++;
	}while((bp < bandplan->noOfToneGroups) && (n < MAX_NUM_VECTRD_CMDS_SEQ));
	CmdGrp->numCmds=n;
	CmdGrp->lastCmdNm=-1;
	__SoftDslPrintf(gDslVars,"CreateVectorReadCmdSeq: noOfToneGroups %d gFactor %d MaxGrpSize %d numCmds %d lastStopToneGp %d lastBpEndTone %d",0,
...
Private void G992p3OvhMsgPollPMDPerTonePollCmdControl(void *gDslVars,int lastRspType)
{
	...
	switch(pCmd->cmd[3])
	{
		...
		case kG992p3OvhMsgCmdPMDVectorBlockRead:
		{
			...
			{
				__SoftDslPrintf(gDslVars, "Disable VectorBlockRead subCmd=%d", 0, pCmd->cmd[4]);
...
Public void G992p3OvhMsgTimer(void *gDslVars, unsigned long timeMs)
{
	dslStatusStruct	status;
	int				i;
			...
			if( XdslMibIsVdsl2Mod(gDslVars) ) {
...
#ifdef CONFIG_BCM_DSL_GFAST
							if(XdslMibIsGfastMod(gDslVars))
								pToneData += US_GFAST_TONE_OFFSET;
#endif
							scgIndex = startSCG*g;
							pToneData += scgIndex;
							if (*pMsg != 255)
				...
				case kG992p3OvhMsgCmdPMDSingleRdRsp:
				{
					...
					if(XdslMibIsGfastMod(gDslVars)) {
						...
						pMib->xdslInfo.dirInfo[DS_DIRECTION].lpInfo[path0].INP = (ushort)(ReadCnt16(pMsg) << 1);
						pMsg+=2;
...
#endif
					{
						...
						if(XdslMibIsGinpActive(gDslVars, US_DIRECTION)) {
							...
							if(*pMsg != pMib->xdslInfo.dirInfo[US_DIRECTION].lpInfo[path1].delay) {
								pMib->xdslInfo.dirInfo[US_DIRECTION].lpInfo[path1].delay = *pMsg;
								dslCmd.command = kDslSetUsDataDelay;
								dslCmd.param.value = (long)pMib->xdslInfo.dirInfo[US_DIRECTION].lpInfo[path1].delay;
								(*globalVar.cmdHandlerPtr)(gDslVars, &dslCmd);
							}
							pMsg+=1;
						}
...
#if defined(CONFIG_VDSL_SUPPORTED)
static uchar G992p3OvhMsgGetInitFlags(void *gDslVars)
{
	ulong  bootFlags = AdslCoreGetBootFlags();

	if (globalVar.initResetFlag)
	  return 0;

	return bootFlags & AC_HARDBOOT ? 0x1 : 0x2;
}
		...
		case kG992p3OvhMsgCmdInventory:
			...
			switch (pData[kG992p3CmdSubCode]) {
				...
				case kG992p3OvhMsgCmdInvVdslInitFlagsRq:
					if (XdslMibIsVdsl2Mod(gDslVars)) {
					  pMsg[0] = G992p3OvhMsgGetInitFlags(gDslVars);
					  rspLen = 1;
					  bRsp = true;
					}
					else
		...
		case kDslINMControlCommand:
			...
			switch(pData[kG992p3CmdSubCode]) {
				case kDslSetINMParams:
					{
						unsigned short INMIATO,INMIATS,INMCC,INM_INPEQ_MODE,INM_INPEQ_FORMAT,n;int i;
						...
						if (cmdLen==9) 
						{
							if (pData[8]==0 ||pData[8]==1)
							{	 
								INM_INPEQ_FORMAT=INM_INPEQ_MODE=pData[8];
						...
						if(NULL!=(globalVar.pPLNinpBinIntervalPtr=AdslCoreSharedMemAlloc(n)) )
						{
							for (i=0;i<17;i++)
							{
								if(cmdLen==9 && globalVar.inmConfig.INM_INPEQ_FORMAT==1)
									globalVar.pPLNinpBinIntervalPtr[i]=inpBinIntervalLogFormat[i];
								else
				...
				switch(pData[kG992p3CmdSubCode]) {
								...
								for (j = 0; j < pMib->dsNegBandPlanDiscovery.noOfToneGroups; j++)
								{
									for(;i <= stopSCG; i++) {
										...
										else
										{
											int x, y, z = i*g;
											y = z - g + 1;
											if(y >= pMib->dsNegBandPlanDiscovery.toneGroups[j].startTone) {
												aln = 0;
												for(x = y; x <= z; x++)
													aln += (-pAlnInfo[x] - 33*16);
												aln = (aln/g) >> 3;
											}
											else
												aln = (ulong) ((-pAlnInfo[z] - 33*16) >> 3);
					...
					case kG992p3OvhMsgCmdPMDSingleRead:
					{
						...
						if(XdslMibIsGfastMod(gDslVars)) {
							...
							n = pMib->xdslInfo.dirInfo[US_DIRECTION].lpInfo[path0].INP >> 1;
							if(n > 2047)
								n = 2047;
							WriteCnt16(pMsg, n);
							pMsg+=2;
	...
	uchar segFlagAndId)
{
	...
	DiagWriteString(gLineId(gDslVars), DIAG_DSL_CLIENT, "%s: pFrame=0x%x pBuf=0x%x, pData=0x%x, len=%d, CmdOrRsp = %s, CmdCode = %x\n",
		__FUNCTION__, (long)pFrame, (long)pBuf, (long)pData, len, (kG992p3Cmd == cmdOrRsp )? "CMD":"RSP", pData[kG992p3CmdCode]);
#endif
...
#ifdef GFAST_TESTMODE_TEST
  if ( gFastTestModeReq && !G992p3FrameIsBitSet(gDslVars, globalVar.lastTxCmdFrame, kFrameClearEoc) ) {
    if((gFastTestModeReq & GFAST_START_RTX_TESTMODE) && !G992p3FrameIsBitSet(gDslVars, globalVar.lastTxCmdFrame, kFrameClearEoc) ) {
    ...
    if((gFastTestModeReq & GFAST_STOP_RTX_TESTMODE) && !G992p3FrameIsBitSet(gDslVars, globalVar.lastTxCmdFrame, kFrameClearEoc) ) {
      gfastTestModeRequestCmd[3] = kG992p3OvhMsgCmdEOCGfastStopRtxTestMode;
      if(G992p3OvhMsgXmtSendCmd(gDslVars, gfastTestModeRequestCmd,sizeof(gfastTestModeRequestCmd), NULL,0, true, true))
        gFastTestModeReq &= ~GFAST_STOP_RTX_TESTMODE;
    }
    else
    if((gFastTestModeReq & GFAST_START_TPS_TESTMODE) && !G992p3FrameIsBitSet(gDslVars, globalVar.lastTxCmdFrame, kFrameClearEoc) ) {
      gfastTestModeRequestCmd[3] = kG992p3OvhMsgCmdEOCGfastStartTpsTestMode;
      if(G992p3OvhMsgXmtSendCmd(gDslVars, gfastTestModeRequestCmd,sizeof(gfastTestModeRequestCmd), NULL,0, true, true))
        gFastTestModeReq &= ~GFAST_START_TPS_TESTMODE;
    }
    else
    if((gFastTestModeReq & GFAST_STOP_TPS_TESTMODE) && !G992p3FrameIsBitSet(gDslVars, globalVar.lastTxCmdFrame, kFrameClearEoc) ) {
...
Public void G992p3OvhMsgTimer(void *gDslVars, unsigned long timeQ24ms)
{
}
