...
extern void XdslCoreProcessPrivateSysMediaCfg(ulong mediaSrchCfg);
#endif
		...
		case kOidAdslPrivateSysCtl:
			switch (objId[1]) {
				case kOidAdslPrivateSysMediaCfg:
					if(3 == objIdLen)
						XdslCoreProcessPrivateSysMediaCfg((ulong)objId[2]);
					break;
			}
			break;
#endif
			...
			case kOidAdslPrivate:
				switch (objId[1]) {
					...
					case kOidAdslPrivActiveLineNoise:
						...
						objLen = sizeof (globalVar.activeLineNoise[0]) * globalVar.nTones;
						break;
					case kOidAdslPrivDoiBitAlloc:
						pObj = (void *) globalVar.doiBitAlloc;
						objLen = sizeof (globalVar.doiBitAlloc[0]) * globalVar.nTones;
						break;
					case kOidAdslPrivDoiGain:
						pObj = (void *) globalVar.doiGain;
						objLen = sizeof (globalVar.doiGain[0]) * globalVar.nTones;
						break;
#endif
...
#ifdef CONFIG_BCM_DSL_GFAST
						if(XdslMibIsGfastMod(gDslVars))
							pChanCharLog = globalVar.chanCharLog + US_GFAST_TONE_OFFSET;
						else
#endif
...
#ifdef CONFIG_BCM_DSL_GFAST
						if(XdslMibIsGfastMod(gDslVars))
							pQln = globalVar.quietLineNoise + US_GFAST_TONE_OFFSET;
						else
#endif
...
#ifdef CONFIG_BCM_DSL_GFAST
						if(XdslMibIsGfastMod(gDslVars))
							pSnr = globalVar.snr + US_GFAST_TONE_OFFSET;
						else
#endif
							pSnr = globalVar.snr;
