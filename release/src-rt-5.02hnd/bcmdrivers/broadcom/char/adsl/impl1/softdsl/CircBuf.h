...
#define	SB_CONV_LONG(x)		((char *)ADSL_ENDIAN_CONV_LONG((unsigned long)x))
#else
#define	SB_CONV_LONG(x)		x
#endif

#define	StretchBufferGetSize(pSB)		(SB_CONV_LONG((pSB)->pEnd) - SB_CONV_LONG((pSB)->pStart))
#define	StretchBufferGetStartPtr(pSB)	((void *) SB_CONV_LONG((pSB)->pStart))
#define	StretchBufferGetReadPtr(pSB)	((void *) SB_CONV_LONG((pSB)->pRead))
#define	StretchBufferGetWritePtr(pSB)	((void *) SB_CONV_LONG((pSB)->pWrite))
...
{
	char *wrPtr = SB_CONV_LONG(fBuf->pWrite);
...
{
	char *wrPtr = SB_CONV_LONG(fBuf->pWrite);
	char *rdPtr = SB_CONV_LONG(fBuf->pRead);
...
static __inline int _HostStretchBufferGetReadAvailTotal(stretchBufferStruct *fBuf, char *rdPtr)
{
...
static __inline int HostStretchBufferGetReadAvail(stretchBufferStruct *fBuf)
{
	char *wrPtr = fBuf->pWrite;
	char *rdPtr = fBuf->pRead;
	return( ((wrPtr - rdPtr) >= 0) ? wrPtr - rdPtr: fBuf->pStretchEnd - rdPtr );
}
static __inline int _HostStretchBufferGetWriteAvail(stretchBufferStruct *fBuf, char *rdPtr)
{
	char *wrPtr = fBuf->pWrite;
