...
#define MAX_TONE_NUM    (kVdslMibMaxToneNum + kVdslMibMaxToneNum)    /* 5-band bonding */
#else
...
static short                   gActiveLineNoise[kVdslMibMaxToneNum];    /* No bonding for G.fast */
static uchar                   gDoiBitAlloc[kVdslMibMaxToneNum];
static short                   gDoiGain[kVdslMibMaxToneNum];
#endif
#if defined(CONFIG_BCM963138) || defined(CONFIG_BCM963148)
static short                   gQuietLineNoiseRnc[MAX_TONE_NUM];
#endif
#if defined(SUPPORT_SELT) || defined(CONFIG_VDSLBRCMPRIV1_SUPPORT)
...
#ifdef SUPPORT_SELT
extern void BcmAdslCoreSetSeltNextMode(uchar lineId);
#endif
...
Private void Adsl2MibSetConnectionInfo(void *gDslVars, G992p3CodingParams *param, long code, long val, adsl2ConnectionInfo *pConInfo)
{
    ...
    pInfo->L = ADSL_ENDIAN_CONV_ULONG(param->L);
...
Private void Xdsl2MibSetConnectionInfo(void *gDslVars, FramerDeframerOptions *param, int dir, int msgLen)
{
   ...
   if(US_DIRECTION == dir) {
      ...
      if(param->codingType & kAdslTrellisOn)
         ...
         pXdslInfo->trellisCoding2 &= ~kAdsl2TrellisTxEnabled;
      if(!XdslMibIsGfastMod(gDslVars) && (param->fireEnabled & kFireUsEnabled)) {
      ...
      if(param->codingType & kAdslTrellisOn)
         ...
         pXdslInfo->trellisCoding2 &= ~kAdsl2TrellisRxEnabled;
      if(!XdslMibIsGfastMod(gDslVars) && (param->fireEnabled & kFireDsEnabled)){
...
#if defined(CONFIG_VDSLBRCMPRIV1_SUPPORT) && defined(SUPPORT_DSL_BONDING)
...
Public void AdslMibTimer(void *gDslVars, unsigned long timeMs)
{
    unsigned long   sec;
    int path0, path1;
    ...
    if(globalVar.txShowtimeTime>3000 && globalVar.txUpdateStatFlag==1 )
    {
        ...
        if(sec > globalVar.secElapsedShTm+1)
        {
            ...
            if(XdslMibIsXdsl2Mod(gDslVars))
            {
                ...
                do_div(var64, 1000);
                globalVar.adslMib.xdslStat[path0].xmtStat.cntRS = (ulong)var64;
                if(!XdslMibIsGinpActive(gDslVars, US_DIRECTION) ||
                    !XdslMibIsDataCarryingPath(gDslVars, path0, US_DIRECTION)) {    /* OHM Channel */
                    ...
                    globalVar.adslMib.xdslStat[path0].xmtStat.cntSF = (ulong)var64;
                }
                ...
                if(globalVar.adslMib.lp2TxActive)
                {
                    ...
                    do_div(var64, 1000);
                    globalVar.adslMib.xdslStat[path1].xmtStat.cntRS = (ulong)var64;
                    if(!XdslMibIsGinpActive(gDslVars, US_DIRECTION) ||
                        !XdslMibIsDataCarryingPath(gDslVars, path1, US_DIRECTION)) {    /* OHM Channel */
                        var64 = globalVar.txShowtimeTime << 8;
                        ...
                        globalVar.adslMib.xdslStat[path1].xmtStat.cntSF = (ulong)var64;
                    }
                }
            }
            else
            {
                ...
                globalVar.adslMib.adslStat.xmtStat.cntSF = (ulong)var64;
            }
        }
    }
    ...
    if (sec != 0) {
#ifdef DELAY_LOS_COUNT
        if (globalVar.currSecondLOSPending)
          AdslMibUpdateLOS(gDslVars); /* for previous second */
        globalVar.currSecondLOSPending = false;
#endif
...
#ifdef CONFIG_BCM_DSL_GFAST
            && !globalVar.adslMib.fastRetrainActive
#endif
            )
        {
...
#ifndef DELAY_LOS_COUNT
                AdslMibUpdateLOS(gDslVars);
#else
                globalVar.currSecondLOSPending = true;
#endif
            }
            if (!globalVar.uasOnSes)
...
Private int XdslMibGetMaxToneNum(void *gDslVars)
{
    ...
    if(kVdslProfileBrcmPriv1 == globalVar.adslMib.xdslInfo.vdsl2Profile)
        n = MAX_TONE_NUM;
    else
#endif
        n = MAX_TONE_NUM/2;
    ...
    case kDslDspControlStatus:
      switch(status->param.dslConnectInfo.code)
      {
        ...
        case kFireMonitoringCounters:
        {
          ...
          pFireStat->reXmtRSCodewordsRcved = ADSL_ENDIAN_CONV_LONG(pLong[kFireReXmtRSCodewordsRcved]);
          pFireStat->reXmtUncorrectedRSCodewords = ADSL_ENDIAN_CONV_LONG(pLong[kFireReXmtUncorrectedRSCodewords]);
          pFireStat->reXmtCorrectedRSCodewords = ADSL_ENDIAN_CONV_LONG(pLong[kFireReXmtCorrectedRSCodewords]);
          break;
        }
...
#ifdef SUPPORT_SELT
          BcmAdslCoreSetSeltNextMode(gLineId(gDslVars));
#endif
          break;
      ...
      switch (status->param.dslTrainingInfo.code) {
        ...
        case kG992FireState:
          ...
          if (0 == (val & ~(kFireDsEnabled | kFireUsEnabled))) {
            globalVar.adslMib.xdslInfo.dirInfo[DS_DIRECTION].lpInfo[0].rtxMode = (val & kFireDsEnabled) ? 1 : 0;
            globalVar.adslMib.xdslInfo.dirInfo[US_DIRECTION].lpInfo[0].rtxMode = (val & kFireUsEnabled) ? 1 : 0;
          }
          break;
#ifdef CONFIG_BCM_DSL_GFAST
...
#if defined(CONFIG_VDSLBRCMPRIV1_SUPPORT) && defined(SUPPORT_DSL_BONDING)
            if((kVdslProfileBrcmPriv1 == globalVar.adslMib.xdslInfo.vdsl2Profile) && (1 == gLineId(gDslVars)))
        ...
        case kG992DataRcvDetectLOS:
            ...
            globalVar.adslMib.adslPhys.adslCurrStatus &= ~(kAdslPhysStatusLOR | kAdslPhysStatusLOF | kAdslPhysStatusLOM | kAdslPhysStatusNoDefect);
    ...
    case kDslConnectInfoStatus:
      ...
      switch (status->param.dslConnectInfo.code) {
        ...
        case kG992p2XmtCodingParamsInfo:
            ...
            codingParam->direction = ADSL_ENDIAN_CONV_LONG(codingParam->direction);
        ...
        case kG992ShowtimeMonitoringStatus:
          {
...
#ifdef  ADSLDRV_LITTLE_ENDIAN
            BlockLongMoveReverse(DSL_COUNTERS_MAX, counters, counters);
#endif
            pathId = XdslMibGetCurrentMappedPathId(gDslVars, DS_DIRECTION);
            ...
            if((XdslMibIsGinpActive(gDslVars, DS_DIRECTION) || XdslMibIsGinpActive(gDslVars, US_DIRECTION)) &&
              !(XdslMibIsGinpActive(gDslVars, DS_DIRECTION) && XdslMibIsGinpActive(gDslVars, US_DIRECTION))) {
    ...
    case kAtmStatus:
      ...
      switch (status->param.atmStatus.code) {
          ...
          case kAtmStatCounters:
              ...
              p = &counters;
              BlockLongMoveReverse(sizeof(atmPhyCounters)>>2, (long *) status->param.atmStatus.param.value, (long *)p);	/* id and bertStatus(first 2 ushort) are not used */
#else
    ...
    case kDslReceivedEocCommand:
      switch(status->param.dslClearEocMsg.msgId) {
        ...
        case kDslShowtimeSnrMarginHdrQ8:
        {
          ...
          long avgSNRMargin = ADSL_ENDIAN_CONV_LONG(pSnrMarginInfoType->avgSNRMargin) >> 4;  /* Q8  -->  Q4 */
        ...
        case kGinpMonitoringCounters:
        {
          ...
          BlockLongMoveReverse(sizeof(GinpCounters) >> 2, (long *)pGinpCounter, (long *)&ginpCounter);
        ...
        case kGfastEocMonitoringCounters:
        {
          ...
          BlockLongMoveReverse(sizeof(gfastEocCounters) >> 2, (long *)pEocCounters, (long *)&eocCounters);
        ...
        case kDslGfastVectoringEocSegments:
        {
          ...
          BlockLongMoveReverse(sizeof(GfastTxVectorFBEocSegment) >> 2, (long *)pVectorFBCounters, (long *)&vectorFBCounters);
        ...
        case kDsl993p2FramerDeframerDs:
        {
          ...
          /* a ulong, ETR_kbps was added */
          if(n >= GINP_FRAMER_ETR_STRUCT_SIZE)
            pFramerParam->ETR_kbps = ADSL_ENDIAN_CONV_LONG(pFramerParam->ETR_kbps);
          ...
          if(n >= L32_FRAMER_STRUCT_SIZE)
            pFramerParam->L = ADSL_ENDIAN_CONV_LONG(pFramerParam->L);
          /* GFAST: maxMemory, ndr, Ldr, Nret */
          if(n >= Nret_FRAMER_STRUCT_SIZE) {
            pFramerParam->maxMemory = ADSL_ENDIAN_CONV_LONG(pFramerParam->maxMemory);
            pFramerParam->ndr = ADSL_ENDIAN_CONV_LONG(pFramerParam->ndr);
            pFramerParam->Ldr = ADSL_ENDIAN_CONV_SHORT(pFramerParam->Ldr);
          }
          if(n >= sizeof(FramerDeframerOptions)) {
            pFramerParam->etru = ADSL_ENDIAN_CONV_LONG(pFramerParam->etru);
            pFramerParam->Lmax = ADSL_ENDIAN_CONV_LONG(pFramerParam->Lmax);
            pFramerParam->Lmin = ADSL_ENDIAN_CONV_LONG(pFramerParam->Lmin);
            pFramerParam->ETRminEoc = ADSL_ENDIAN_CONV_LONG(pFramerParam->ETRminEoc);
          }
#endif
          if(n < L32_FRAMER_STRUCT_SIZE)
        ...
        {
          long val = ADSL_ENDIAN_CONV_LONG(*(long *)status->param.dslClearEocMsg.dataPtr);
#ifdef USE_TRAINING_ATTNDR
          long newAttndr = ActualRate(val*1000);
          long halfLastAttndr = ((globalVar.adslMib.xdslPhys.adslCurrAttainableRate)>>11); /* Hz to kHz and divide by 2 (div by 2048) */
          pathId = XdslMibGetCurrentMappedPathId(gDslVars, DS_DIRECTION);
      ...
      case kDslNtrCounters:
      {
         ...
         BlockLongMoveReverse(ntrCntLen>>2, (long *)pNtr, (long *)&globalVar.adslMib.ntrCnt);
      }
         break;
#endif
#ifdef CONFIG_VDSL_SUPPORTED
      ...
      case kDsl993p2BandPlanDsDump:
      {
        ...
        if(globalVar.waitBandPlan)
        {
          ...
          if(XdslMibIsGfastMod(gDslVars)) {
            ...
            BlockByteClear(sizeof(globalVar.doiBitAlloc[0])*globalVar.nTones, (void*)globalVar.doiBitAlloc);
            BlockByteClear(sizeof(globalVar.doiGain[0])*globalVar.nTones, (void*)globalVar.doiGain);
          }
#endif
        ...
        if(globalVar.bandPlanType==MedleyPhase)
        {
          if(bp->reserved==PhyBandPlan)
          {
            ...
            __SoftDslPrintf(gDslVars, "Physical Gfactor_MEDLEYSETds=%d",0,globalVar.adslMib.physGfactors.Gfactor_MEDLEYSETds);
          }
        ...
        else
        {
          if(bp->reserved==PhyBandPlan)
          {
            ...
            globalVar.dsPhysGfactor = &globalVar.adslMib.physGfactors.Gfactor_SUPPORTERCARRIERSds;
            __SoftDslPrintf(gDslVars, "Physical Gfactor_SUPPORTERCARRIERSds=%d",0,globalVar.adslMib.physGfactors.Gfactor_SUPPORTERCARRIERSds);
          }
          else if (bp->reserved==NegBandPlan)
          {
      ...
      case kDsl993p2TestHlin:
          ...
          if(n > nMaxToneAllow)
             n = nMaxToneAllow;
          BlockLongMoveReverse(n<<1,(long *)(status->param.dslClearEocMsg.dataPtr),(long *)globalVar.uer);
      break;
#ifdef SUPPORT_SELT
...
#endif
      {
        int i,k,n; uchar *pBuf, *pBitAlloc;
        pBuf =(uchar *)(status->param.dslClearEocMsg.dataPtr); 
        k=0;
#ifdef CONFIG_BCM_DSL_GFAST
        if(XdslMibIsGfastMod(gDslVars)) {
            if(kDsl993p2DOIFeBi == status->param.dslClearEocMsg.msgId)
                pBitAlloc = globalVar.doiBitAlloc + US_GFAST_TONE_OFFSET;
            else
                pBitAlloc = globalVar.bitAlloc + US_GFAST_TONE_OFFSET;
        }
        else
#endif
      ...
      case kDsl993p2FeBiPhy: /* VDSL Only */
      {
          ...
          for(n = 0; n < globalVar.adslMib.usPhyBandPlan.noOfToneGroups; n++) {
              if(globalVar.adslMib.usPhyBandPlan.toneGroups[n].endTone >= globalVar.nTones)
                  break;
      ...
      case kDsl993p2BitSwapTones :
      {
        ...
        if(msgLen > MAX_BITSWAP_TONES)
            msgLen = MAX_BITSWAP_TONES;
        for(i=0;i<msgLen;i++)
        {
            k = 0;
...
#endif
      {
          ...
          if(XdslMibIsGfastMod(gDslVars)) {
              if(kDsl993p2DOIFeGi == status->param.dslClearEocMsg.msgId)
                  pGain = globalVar.doiGain + US_GFAST_TONE_OFFSET;
              else
                  pGain = globalVar.gain + US_GFAST_TONE_OFFSET;
          }
          else
#endif
      ...
      {
        unsigned long val = ADSL_ENDIAN_CONV_LONG(*(unsigned long *)status->param.dslClearEocMsg.dataPtr);
      ...
      case kDsl993p2MaxRate:
      {
        ...
        globalVar.adslMib.adslPhys.adslCurrAttainableRate = ActualRate(ADSL_ENDIAN_CONV_LONG(*pUlong)) * 1000;
        globalVar.adslMib.adslAtucPhys.adslCurrAttainableRate = ActualRate(ADSL_ENDIAN_CONV_LONG(*(pUlong+1))) * 1000;
#ifdef USE_TRAINING_ATTNDR
        globalVar.adslMib.xdslPhys.adslTrainAttainableRate = ActualRate(ADSL_ENDIAN_CONV_LONG(*pUlong)) * 1000;
        globalVar.adslMib.xdslPhys.adslShowAttainableRate = 0;
#endif
      }
        break;
...
#ifdef USE_LOCAL_DS_POWER
        AdslMibUpdateACTPSD(gDslVars, valQ4, DS_DIRECTION);
#endif
      ...
      {
          int n,i,grpIndex; uchar *pMsg; short val=0, *pQln = globalVar.quietLineNoise;
      ...
      {
          int n,i,grpIndex; uchar *pMsg; short val=0, *pChanCharLog = globalVar.chanCharLog;
          int g=globalVar.adslMib.gFactors.Gfactor_SUPPORTERCARRIERSus;
          if(0 == g)
              g=1;
...
#ifdef CONFIG_BCM_DSL_GFAST
          if(XdslMibIsGfastMod(gDslVars))
              pChanCharLog += US_GFAST_TONE_OFFSET;
#endif
          for(n=0;n<globalVar.adslMib.usNegBandPlanDiscovery32.noOfToneGroups;n++)
          {
              grpIndex=-1;
      ...
      {
          int n,i,grpIndex; uchar *pMsg; short val=0, *pSnr = globalVar.snr;
...
#ifdef CONFIG_BCM_DSL_GFAST
          if(XdslMibIsGfastMod(gDslVars))
              pChanCharLin = globalVar.chanCharLin + US_GFAST_TONE_OFFSET;
          else
#endif
