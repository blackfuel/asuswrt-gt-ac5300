...
Public Boolean G997SetTxBuffer(void *gDslVars, ulong len, void *bufPtr)
{
	...
	DiagWriteString(gLineId(gDslVars), DIAG_DSL_CLIENT, "%s: line%d - bufPtr=0x%08x len=%d\n", __FUNCTION__, gLineId(gDslVars), (unsigned int)bufPtr, len);
...
Public void G997Timer(void *gDslVars, unsigned long timeMs)
{
