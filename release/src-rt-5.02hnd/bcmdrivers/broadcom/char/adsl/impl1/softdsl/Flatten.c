...
#define	CHECK_CMD_BUF_AVAIL(szReq,szAvail)	if ((long)(szReq) > (long)(szAvail)) return 0;
	...
	*dstPtr++ = ADSL_ENDIAN_CONV_LONG((ulong)cmd->command);
	switch (DSL_COMMAND_CODE(cmd->command)) {
...
#endif
			*dstPtr++ = ADSL_ENDIAN_CONV_LONG((ulong)cmd->param.dslModeSpec.direction);
			*dstPtr++ = ADSL_ENDIAN_CONV_LONG((ulong)cmd->param.dslModeSpec.capabilities.modulations);
			*dstPtr++ = ADSL_ENDIAN_CONV_LONG((ulong)cmd->param.dslModeSpec.capabilities.minDataRate);
			*dstPtr++ = ADSL_ENDIAN_CONV_LONG((ulong)cmd->param.dslModeSpec.capabilities.maxDataRate);
#if	defined(G992P2) || (defined(G992P1_ANNEX_A) && defined(G992P1_ANNEX_A_USED_FOR_G992P2))
			if (ADSL_PHY_SUPPORT(kAdslPhyG992p2Init)) {
				*dstPtr++ = ADSL_ENDIAN_CONV_LONG((ulong)cmd->param.dslModeSpec.capabilities.carrierInfoG992p2.downstreamMinCarr);
				*dstPtr++ = ADSL_ENDIAN_CONV_LONG((ulong)cmd->param.dslModeSpec.capabilities.carrierInfoG992p2.downstreamMaxCarr);
				*dstPtr++ = ADSL_ENDIAN_CONV_LONG((ulong)cmd->param.dslModeSpec.capabilities.carrierInfoG992p2.upstreamMinCarr);
				*dstPtr++ = ADSL_ENDIAN_CONV_LONG((ulong)cmd->param.dslModeSpec.capabilities.carrierInfoG992p2.upstreamMaxCarr);
			}
#endif
#ifdef	G992P1_ANNEX_A
			if (ADSL_PHY_SUPPORT(kAdslPhyAnnexA)) {
				*dstPtr++ = ADSL_ENDIAN_CONV_LONG((ulong)cmd->param.dslModeSpec.capabilities.carrierInfoG992p1.downstreamMinCarr);
				*dstPtr++ = ADSL_ENDIAN_CONV_LONG((ulong)cmd->param.dslModeSpec.capabilities.carrierInfoG992p1.downstreamMaxCarr);
				*dstPtr++ = ADSL_ENDIAN_CONV_LONG((ulong)cmd->param.dslModeSpec.capabilities.carrierInfoG992p1.upstreamMinCarr);
				*dstPtr++ = ADSL_ENDIAN_CONV_LONG((ulong)cmd->param.dslModeSpec.capabilities.carrierInfoG992p1.upstreamMaxCarr);
				*dstPtr++ = ADSL_ENDIAN_CONV_LONG((ulong)cmd->param.dslModeSpec.capabilities.subChannelInfo);
			}
#endif
#ifdef	G992P1_ANNEX_B
			if (ADSL_PHY_SUPPORT(kAdslPhyAnnexB) || ADSL_PHY_SUPPORT(kAdslPhySADSL)) {
				*dstPtr++ = ADSL_ENDIAN_CONV_LONG((ulong)cmd->param.dslModeSpec.capabilities.carrierInfoG992p1AnnexB.downstreamMinCarr);
				*dstPtr++ = ADSL_ENDIAN_CONV_LONG((ulong)cmd->param.dslModeSpec.capabilities.carrierInfoG992p1AnnexB.downstreamMaxCarr);
				*dstPtr++ = ADSL_ENDIAN_CONV_LONG((ulong)cmd->param.dslModeSpec.capabilities.carrierInfoG992p1AnnexB.upstreamMinCarr);
				*dstPtr++ = ADSL_ENDIAN_CONV_LONG((ulong)cmd->param.dslModeSpec.capabilities.carrierInfoG992p1AnnexB.upstreamMaxCarr);
				*dstPtr++ = ADSL_ENDIAN_CONV_LONG((ulong)cmd->param.dslModeSpec.capabilities.subChannelInfoAnnexB);
			}
#endif
#ifdef	G992_ANNEXC
			*dstPtr++ = ADSL_ENDIAN_CONV_LONG((ulong)cmd->param.dslModeSpec.capabilities.carrierInfoG992p1AnnexC.downstreamMinCarr);
			*dstPtr++ = ADSL_ENDIAN_CONV_LONG((ulong)cmd->param.dslModeSpec.capabilities.carrierInfoG992p1AnnexC.downstreamMaxCarr);
			*dstPtr++ = ADSL_ENDIAN_CONV_LONG((ulong)cmd->param.dslModeSpec.capabilities.carrierInfoG992p1AnnexC.upstreamMinCarr);
			*dstPtr++ = ADSL_ENDIAN_CONV_LONG((ulong)cmd->param.dslModeSpec.capabilities.carrierInfoG992p1AnnexC.upstreamMaxCarr);
			*dstPtr++ = ADSL_ENDIAN_CONV_LONG((ulong)cmd->param.dslModeSpec.capabilities.subChannelInfoAnnexC);
#endif
#ifdef	G992P1_ANNEX_I
			*dstPtr++ = ADSL_ENDIAN_CONV_LONG((ulong)cmd->param.dslModeSpec.capabilities.carrierInfoG992p1AnnexI.downstreamMinCarr);
			*dstPtr++ = ADSL_ENDIAN_CONV_LONG((ulong)cmd->param.dslModeSpec.capabilities.carrierInfoG992p1AnnexI.downstreamMaxCarr);
			*dstPtr++ = ADSL_ENDIAN_CONV_LONG((ulong)cmd->param.dslModeSpec.capabilities.carrierInfoG992p1AnnexI.upstreamMinCarr);
			*dstPtr++ = ADSL_ENDIAN_CONV_LONG((ulong)cmd->param.dslModeSpec.capabilities.carrierInfoG992p1AnnexI.upstreamMaxCarr);
			*dstPtr++ = ADSL_ENDIAN_CONV_LONG((ulong)cmd->param.dslModeSpec.capabilities.subChannelInfoAnnexI);
#endif
#ifdef	G992P5
			if (ADSL_PHY_SUPPORT(kAdslPhyAdsl2p)) {
				*dstPtr++ = ADSL_ENDIAN_CONV_LONG((ulong)cmd->param.dslModeSpec.capabilities.carrierInfoG992p5.downstreamMinCarr);
				*dstPtr++ = ADSL_ENDIAN_CONV_LONG((ulong)cmd->param.dslModeSpec.capabilities.carrierInfoG992p5.downstreamMaxCarr);
				*dstPtr++ = ADSL_ENDIAN_CONV_LONG((ulong)cmd->param.dslModeSpec.capabilities.carrierInfoG992p5.upstreamMinCarr);
				*dstPtr++ = ADSL_ENDIAN_CONV_LONG((ulong)cmd->param.dslModeSpec.capabilities.carrierInfoG992p5.upstreamMaxCarr);
				*dstPtr++ = ADSL_ENDIAN_CONV_LONG((ulong)cmd->param.dslModeSpec.capabilities.subChannelInfop5);
			}
#endif
			*dstPtr++ = ADSL_ENDIAN_CONV_LONG((ulong)cmd->param.dslModeSpec.capabilities.features);
			*dstPtr++ = ADSL_ENDIAN_CONV_LONG((ulong)cmd->param.dslModeSpec.capabilities.auxFeatures);
			*dstPtr++ = ADSL_ENDIAN_CONV_LONG((ulong)cmd->param.dslModeSpec.capabilities.demodCapabilities);
			*dstPtr++ = ADSL_ENDIAN_CONV_LONG((ulong)cmd->param.dslModeSpec.capabilities.noiseMargin);
#ifdef G992_ATUC /* last please, for ATU-C only */
			*dstPtr++ = ADSL_ENDIAN_CONV_LONG((ulong)cmd->param.dslModeSpec.capabilities.xmtRSf);
			*dstPtr++ = ADSL_ENDIAN_CONV_LONG((ulong)cmd->param.dslModeSpec.capabilities.xmtRS);
			*dstPtr++ = ADSL_ENDIAN_CONV_LONG((ulong)cmd->param.dslModeSpec.capabilities.xmtS);
			*dstPtr++ = ADSL_ENDIAN_CONV_LONG((ulong)cmd->param.dslModeSpec.capabilities.xmtD);
			*dstPtr++ = ADSL_ENDIAN_CONV_LONG((ulong)cmd->param.dslModeSpec.capabilities.rcvRSf);
			*dstPtr++ = ADSL_ENDIAN_CONV_LONG((ulong)cmd->param.dslModeSpec.capabilities.rcvRS);
			*dstPtr++ = ADSL_ENDIAN_CONV_LONG((ulong)cmd->param.dslModeSpec.capabilities.rcvS);
			*dstPtr++ = ADSL_ENDIAN_CONV_LONG((ulong)cmd->param.dslModeSpec.capabilities.rcvD);
#endif
#ifdef G992P3
			if (ADSL_PHY_SUPPORT(kAdslPhyAdsl2)) {
#ifdef FLATTEN_ADDR_ADJUST
				*dstPtr++ = (ulong) (ADSL_ENDIAN_CONV_LONG(SDRAM_ADDR_TO_ADSL(cmd->param.dslModeSpec.capabilities.carrierInfoG992p3AnnexA)));
#else
				*dstPtr++ = ADSL_ENDIAN_CONV_LONG((ulong)cmd->param.dslModeSpec.capabilities.carrierInfoG992p3AnnexA);
#endif
			}
#endif
#ifdef G992P5
			if (ADSL_PHY_SUPPORT(kAdslPhyAdsl2p)) {
#ifdef FLATTEN_ADDR_ADJUST
				*dstPtr++ = (ulong) (ADSL_ENDIAN_CONV_LONG(SDRAM_ADDR_TO_ADSL(cmd->param.dslModeSpec.capabilities.carrierInfoG992p5AnnexA)));
#else
				*dstPtr++ = ADSL_ENDIAN_CONV_LONG((ulong)cmd->param.dslModeSpec.capabilities.carrierInfoG992p5AnnexA);
#endif
			}
#endif
#ifdef G993
...
#ifdef FLATTEN_ADDR_ADJUST
				*dstPtr++ = (ulong) (ADSL_ENDIAN_CONV_LONG(SDRAM_ADDR_TO_ADSL(cmd->param.dslModeSpec.capabilities.carrierInfoG993p2AnnexA)));
#else
				*dstPtr++ = ADSL_ENDIAN_CONV_LONG((ulong)cmd->param.dslModeSpec.capabilities.carrierInfoG993p2AnnexA);
#endif
			}
#endif
			break;
		...
		case kDslDiagSetupCmd:
			...
			*dstPtr++ = ADSL_ENDIAN_CONV_LONG((ulong)cmd->param.dslDiagSpec.setup);
			*dstPtr++ = ADSL_ENDIAN_CONV_LONG(cmd->param.dslDiagSpec.eyeConstIndex1);
			*dstPtr++ = ADSL_ENDIAN_CONV_LONG(cmd->param.dslDiagSpec.eyeConstIndex2);
			*dstPtr++ = ADSL_ENDIAN_CONV_LONG(cmd->param.dslDiagSpec.logTime);
			break;
			
		case kDslDiagSetupBufDesc:
			CHECK_CMD_BUF_AVAIL(2,statAvail-1);
			*dstPtr++ = ADSL_ENDIAN_CONV_LONG(SDRAM_ADDR_TO_ADSL(cmd->param.dslDiagBufDesc.descBufAddr));
			*dstPtr++ = ADSL_ENDIAN_CONV_LONG(cmd->param.dslDiagBufDesc.bufCnt);
			break;

		case kDslTestCmd:
			*dstPtr++ = ADSL_ENDIAN_CONV_LONG((ulong)cmd->param.dslTestSpec.type);
			if (kDslTestToneSelection == cmd->param.dslTestSpec.type) {
				...
				*dstPtr++ = ADSL_ENDIAN_CONV_LONG((ulong)cmd->param.dslTestSpec.param.toneSelectSpec.xmtStartTone);
				*dstPtr++ = ADSL_ENDIAN_CONV_LONG((ulong)cmd->param.dslTestSpec.param.toneSelectSpec.xmtNumOfTones);
				*dstPtr++ = ADSL_ENDIAN_CONV_LONG((ulong)cmd->param.dslTestSpec.param.toneSelectSpec.rcvStartTone);
				*dstPtr++ = ADSL_ENDIAN_CONV_LONG((ulong)cmd->param.dslTestSpec.param.toneSelectSpec.rcvNumOfTones);
				...
				*dstPtr++ = ADSL_ENDIAN_CONV_LONG((ulong) cmd->param.dslTestSpec.param.value);
			}
				...
				*dstPtr++ = ADSL_ENDIAN_CONV_LONG((ulong)cmd->param.dslTestSpec.param.marginTweakSpec.extraPowerRequestQ4dB);
				*dstPtr++ = ADSL_ENDIAN_CONV_LONG((ulong)cmd->param.dslTestSpec.param.marginTweakSpec.numOfCarriers);
...
#ifdef FLATTEN_ADDR_ADJUST
			*dstPtr++ = ADSL_ENDIAN_CONV_ULONG(SDRAM_ADDR_TO_ADSL(cmd->param.dslStatusBufSpec.pBuf));
#else
			*dstPtr++ = ADSL_ENDIAN_CONV_LONG((ulong) cmd->param.dslStatusBufSpec.pBuf);
#endif
			*dstPtr++ = ADSL_ENDIAN_CONV_LONG(cmd->param.dslStatusBufSpec.bufSize);
			break;
		...
		case kDslAfeTestCmd:
			...
			*dstPtr++ = ADSL_ENDIAN_CONV_LONG(cmd->param.dslAfeTestSpec.type);
#ifdef FLATTEN_ADDR_ADJUST
			*dstPtr++ = (ulong) (ADSL_ENDIAN_CONV_LONG(SDRAM_ADDR_TO_ADSL(cmd->param.dslAfeTestSpec.afeParamPtr)));
#else
			*dstPtr++ = ADSL_ENDIAN_CONV_LONG((ulong) cmd->param.dslAfeTestSpec.afeParamPtr);
#endif
			*dstPtr++ = ADSL_ENDIAN_CONV_LONG(cmd->param.dslAfeTestSpec.afeParamSize);
#ifdef FLATTEN_ADDR_ADJUST
			*dstPtr++ = (ulong) (ADSL_ENDIAN_CONV_LONG(SDRAM_ADDR_TO_ADSL(cmd->param.dslAfeTestSpec.imagePtr)));
#else
			*dstPtr++ = ADSL_ENDIAN_CONV_LONG((ulong) cmd->param.dslAfeTestSpec.imagePtr);
#endif
			*dstPtr++ = ADSL_ENDIAN_CONV_LONG(cmd->param.dslAfeTestSpec.imageSize);
			break;

		case kDslAfeTestCmd1:
			CHECK_CMD_BUF_AVAIL(3,statAvail-1);
			*dstPtr++ = ADSL_ENDIAN_CONV_LONG(cmd->param.dslAfeTestSpec1.type);
			*dstPtr++ = ADSL_ENDIAN_CONV_LONG(cmd->param.dslAfeTestSpec1.param1);
			*dstPtr++ = ADSL_ENDIAN_CONV_LONG(cmd->param.dslAfeTestSpec1.param2);
			break;

		case kDslPLNControlCmd:
			*dstPtr++ = ADSL_ENDIAN_CONV_LONG(cmd->param.dslPlnSpec.plnCmd);
			if (kDslPLNControlStart == cmd->param.dslPlnSpec.plnCmd) {
				dstPtr[0] = ADSL_ENDIAN_CONV_LONG(cmd->param.dslPlnSpec.mgnDescreaseLevelPerBin);
				dstPtr[1] = ADSL_ENDIAN_CONV_LONG(cmd->param.dslPlnSpec.mgnDescreaseLevelBand);
				dstPtr += 2;
				
			}
			else if(kDslPLNControlDefineInpBinTable==cmd->param.dslPlnSpec.plnCmd) {
				*dstPtr++ = ADSL_ENDIAN_CONV_LONG(cmd->param.dslPlnSpec.nInpBin);
#ifdef FLATTEN_ADDR_ADJUST
				*dstPtr++ = ADSL_ENDIAN_CONV_ULONG(SDRAM_ADDR_TO_ADSL(cmd->param.dslPlnSpec.inpBinPtr));
#else
				*dstPtr++ = ADSL_ENDIAN_CONV_LONG((ulong) cmd->param.dslPlnSpec.inpBinPtr);
#endif
			}
			else if (kDslPLNControlDefineItaBinTable==cmd->param.dslPlnSpec.plnCmd) {
				*dstPtr++ = ADSL_ENDIAN_CONV_LONG(cmd->param.dslPlnSpec.nItaBin);
#ifdef FLATTEN_ADDR_ADJUST
				*dstPtr++ = ADSL_ENDIAN_CONV_ULONG(SDRAM_ADDR_TO_ADSL(cmd->param.dslPlnSpec.itaBinPtr));
#else
				*dstPtr++ = ADSL_ENDIAN_CONV_LONG((ulong) cmd->param.dslPlnSpec.itaBinPtr);
#endif
			}
			else if (kDslINMControlParams==cmd->param.dslPlnSpec.plnCmd) {
				*dstPtr++ = ADSL_ENDIAN_CONV_LONG((ulong) cmd->param.dslPlnSpec.inmContinueConfig);
				*dstPtr++ = ADSL_ENDIAN_CONV_LONG((ulong) cmd->param.dslPlnSpec.inmInpEqMode);
			}
			else if (kDslINMConfigParams==cmd->param.dslPlnSpec.plnCmd) {
				*dstPtr++ = ADSL_ENDIAN_CONV_LONG((ulong) cmd->param.dslPlnSpec.inmContinueConfig);
				*dstPtr++ = ADSL_ENDIAN_CONV_LONG((ulong) cmd->param.dslPlnSpec.inmInpEqMode);
				*dstPtr++ = ADSL_ENDIAN_CONV_LONG((ulong) cmd->param.dslPlnSpec.inmIATO);
				*dstPtr++ = ADSL_ENDIAN_CONV_LONG((ulong) cmd->param.dslPlnSpec.inmIATS);
			}
			else if (kDslINMConfigInpEqFormat == cmd->param.dslPlnSpec.plnCmd)
				*dstPtr++ = ADSL_ENDIAN_CONV_LONG((ulong) cmd->param.dslPlnSpec.inmInpEqFormat);
			break;

#ifdef G997_1
		case kDslSendEocCommand:
			*dstPtr++ = ADSL_ENDIAN_CONV_LONG((ulong)cmd->param.value);
			if (cmd->param.value >= kDslClearEocFirstCmd)
				{
				*dstPtr++ = ADSL_ENDIAN_CONV_LONG(cmd->param.dslClearEocMsg.msgType);
...
#ifdef FLATTEN_ADDR_ADJUST
					*dstPtr++ = (ulong)  (ADSL_ENDIAN_CONV_LONG(SDRAM_ADDR_TO_ADSL(cmd->param.dslClearEocMsg.dataPtr)));
#else
					*dstPtr++ = ADSL_ENDIAN_CONV_LONG((ulong)cmd->param.dslClearEocMsg.dataPtr);
#endif
				}
			break;
#endif
		...
		case kDslOLRRequestCmd:
			...
			*dstPtr++ = ADSL_ENDIAN_CONV_LONG((ulong)cmd->param.dslOLRRequest.msgType);
			*dstPtr++ = ADSL_ENDIAN_CONV_LONG((ulong)cmd->param.dslOLRRequest.nCarrs);
			...
			dstPtr += 1;
#ifdef FLATTEN_ADDR_ADJUST
			*dstPtr++ = (ulong) (ADSL_ENDIAN_CONV_LONG(SDRAM_ADDR_TO_ADSL(cmd->param.dslOLRRequest.carrParamPtr)));
#else
			*dstPtr++ = ADSL_ENDIAN_CONV_LONG((ulong)cmd->param.dslOLRRequest.carrParamPtr);
#endif
			break;

		case kDslPwrMgrCmd:
			*dstPtr++ = ADSL_ENDIAN_CONV_LONG(cmd->param.dslPwrMsg.msgType);
			*dstPtr++ = ADSL_ENDIAN_CONV_LONG((ulong)cmd->param.dslPwrMsg.param.msg.msgLen);
#ifdef FLATTEN_ADDR_ADJUST
			*dstPtr++ = ADSL_ENDIAN_CONV_ULONG(SDRAM_ADDR_TO_ADSL(cmd->param.dslPwrMsg.param.msg.msgData));
#else
			*dstPtr++ = ADSL_ENDIAN_CONV_LONG((ulong)cmd->param.dslPwrMsg.param.msg.msgData);
#endif
			break;
#endif
...
{
	ulong	addr = (ulong) p;
	int		off  = addr - (ulong) adslXfaceData.sdramBaseAddr;

	if (off >= 0)
		p = (void *) ((ulong) &eprol + off);
	return p;
}
	...
	*dstPtr++ = status->code;
	...
	switch	(DSL_STATUS_CODE(status->code)) {
		...
		case kDslAfeTestStatus:
			...
			*dstPtr++ = (ulong) status->param.dslAfeTestStatus.type;
			*dstPtr++ = (ulong) status->param.dslAfeTestStatus.param1;
			*dstPtr++ = (ulong) status->param.dslAfeTestStatus.param2;			
			break;
...
Public int UnflattenStatus (ulong *srcPtr, dslStatusStruct *status)
{
	...
	status->code = ADSL_ENDIAN_CONV_LONG(*srcPtr++);

	switch	(DSL_STATUS_CODE(status->code)) {
		case		kDslTrainingStatus:
			status->param.dslTrainingInfo.code  = ADSL_ENDIAN_CONV_LONG(srcPtr[0]);
			status->param.dslTrainingInfo.value = ADSL_ENDIAN_CONV_LONG(srcPtr[1]);
			srcPtr += 2;
			break;

		case 		kDslDspControlStatus:
			status->param.dslConnectInfo.code  = ADSL_ENDIAN_CONV_LONG(srcPtr[0]);
			status->param.dslConnectInfo.value = ADSL_ENDIAN_CONV_LONG(srcPtr[1]);
#ifdef FLATTEN_ADDR_ADJUST
...
#endif
			status->param.dslConnectInfo.buffPtr = ADSL_ADDR_TO_HOST(ADSL_ENDIAN_CONV_LONG(srcPtr[2]));
#else
			status->param.dslConnectInfo.buffPtr = (void *) ADSL_ENDIAN_CONV_LONG(srcPtr[2]);
#endif
			srcPtr += 3;
#ifndef DEVICENAME
			...
			switch (status->param.dslConnectInfo.code) {
				...
				case	kG992AocMessageExchangeXmtInfo:
					...
					if (NULL != statDataPtr) {
						...
						srcPtr[-1] = ADSL_ENDIAN_CONV_LONG(statDataLen);
					}
					srcPtr += tmp;
					break;
				case	kG992p2XmtCodingParamsInfo:
				...
				case kDslChannelQuietLineNoise:
					{
					ulong	bufAddr = ADSL_ENDIAN_CONV_LONG(*srcPtr++);
					...
					bufAddr = (ulong)(ADSL_ADDR_TO_HOST(bufAddr));
#endif
		...
		case		kDslShowtimeSNRMarginInfo:
			BlockLongMoveReverse (6, (long *) srcPtr, (long *) &status->param.dslShowtimeSNRMarginInfo);
			srcPtr += 6;
...
#ifdef G997_1
		case kDslReceivedEocCommand:
			status->param.value = ADSL_ENDIAN_CONV_LONG(*srcPtr++);
			if (status->param.value >= kDslClearEocFirstCmd)
				{
				status->param.dslClearEocMsg.msgType = ADSL_ENDIAN_CONV_LONG(*srcPtr++);
				if (status->param.dslClearEocMsg.msgType & kDslClearEocMsgDataVolatileMask)
					{
					...
					{
					status->param.dslClearEocMsg.dataPtr = (char*) ADSL_ENDIAN_CONV_LONG(*srcPtr++);
#ifdef FLATTEN_ADDR_ADJUST
...
#ifdef G997_1_FRAMER
		case kDslG997Status:
			status->param.g997Status.code = ADSL_ENDIAN_CONV_LONG(*srcPtr++);
			switch (status->param.g997Status.code) {
				case kDslFramerRxFrameErr:
				case kDslFramerTxFrameErr:
					status->param.g997Status.param.error= ADSL_ENDIAN_CONV_LONG(*srcPtr++);
					break;
			}
			break;
#endif
		...
		case		kAtmStatus:
			status->param.atmStatus.code        = ADSL_ENDIAN_CONV_LONG(*srcPtr++);
			status->param.atmStatus.param.value = ADSL_ENDIAN_CONV_LONG(*srcPtr++);
			switch (status->param.atmStatus.code) {
				case kAtmStatVcCreated:
					status->param.atmStatus.param.vcInfo.vcId				= ADSL_ENDIAN_CONV_LONG(*srcPtr++);
					status->param.atmStatus.param.vcInfo.vci				= ADSL_ENDIAN_CONV_LONG(*srcPtr++);
					status->param.atmStatus.param.vcInfo.aalType			= ADSL_ENDIAN_CONV_LONG(*srcPtr++);
					status->param.atmStatus.param.vcInfo.fwdPeakCellTime	= ADSL_ENDIAN_CONV_LONG(*srcPtr++);
					status->param.atmStatus.param.vcInfo.backPeakCellTime	= ADSL_ENDIAN_CONV_LONG(*srcPtr++);
					break;
				...
				case kAtmStatOamF5SegmentCell:
				case kAtmStatOamF5End2EndCell:
					status->param.atmStatus.param.oamInfo.oamCmd = ADSL_ENDIAN_CONV_LONG(*srcPtr++);
					break;
				...
				case kAtmStatTxPacket:
					...
					status->param.atmStatus.param.frame.vci		= ADSL_ENDIAN_CONV_LONG(*srcPtr++);
					status->param.atmStatus.param.frame.aalType	= ADSL_ENDIAN_CONV_LONG(*srcPtr++);
					status->param.atmStatus.param.frame.length		= ADSL_ENDIAN_CONV_LONG(*srcPtr++);
					frBytes = status->param.atmStatus.param.frame.length;
					if  (frBytes > kMaxFlattenFramelength)
						frBytes = kMaxFlattenFramelength;
				...
				case kAtmStatBertResult:
					status->param.atmStatus.param.bertInfo.errBits = ADSL_ENDIAN_CONV_LONG(*srcPtr++);
					break;
				case kAtmStatCounters:
					...
					if(ADSL_MIPS_LMEM_ADDR(status->param.atmStatus.param.value))
						__SoftDslPrintf(XdslCoreGetDslVars(DSL_LINE_ID(status->code)),
							...
							kAtmStatCounters);
#endif
					status->param.atmStatus.param.value = (long)(ADSL_ADDR_TO_HOST(status->param.atmStatus.param.value));
#endif
					break;
			}
			break;
		...
		case kDslPrintfStatus:
			status->param.dslPrintfMsg.fmt = (void *)ADSL_ENDIAN_CONV_LONG(*srcPtr++);
#ifdef FLATTEN_ADDR_ADJUST
			{
...
#endif
			status->param.dslPrintfMsg.argNum = ADSL_ENDIAN_CONV_LONG(*srcPtr++);
		...
		case kDslExceptionStatus:
			...
			status->param.dslException.argc = (int)ADSL_ENDIAN_CONV_LONG(*srcPtr++);
		...
		case kDslOemDataAddrStatus:
			...
			status->param.value = (long)(ADSL_ADDR_TO_HOST(status->param.value));
#endif
			break;

		case kDslDataAvailStatus:
			status->param.dslDataAvail.dataPtr = (void *) ADSL_ENDIAN_CONV_LONG(*srcPtr++);
#ifdef FLATTEN_ADDR_ADJUST
#ifdef CHECK_LMEM_ADDR_IN_STAT
			if(ADSL_MIPS_LMEM_ADDR(status->param.dslDataAvail.dataPtr))
				__SoftDslPrintf(XdslCoreGetDslVars(DSL_LINE_ID(status->code)),
					"*** LMEM ADDR in status: code=%ld, value=%ld", 0,
					DSL_STATUS_CODE(status->code),
					(ulong)status->param.dslDataAvail.dataPtr);
#endif
			status->param.dslDataAvail.dataPtr = ADSL_ADDR_TO_HOST(status->param.dslDataAvail.dataPtr);
#endif
			status->param.dslDataAvail.dataLen = ADSL_ENDIAN_CONV_LONG(*srcPtr++);
			break;

#ifdef G992P3
		case kDslOLRRequestStatus:
		case kDslOLRBitGainUpdateStatus:
			status->param.dslOLRRequest.msgType = ADSL_ENDIAN_CONV_LONG(*srcPtr++);
			status->param.dslOLRRequest.nCarrs	= ADSL_ENDIAN_CONV_LONG(*srcPtr++);
			BlockByteMove (12, (void *) srcPtr, (void *) status->param.dslOLRRequest.L);
#ifdef ADSLDRV_LITTLE_ENDIAN
		{
			ulong *pL = (void *) status->param.dslOLRRequest.L;	/* Endian converting "ushort  L[4]" */
			pL[0] = ADSL_ENDIAN_CONV_2SHORTS(pL[0]);
			pL[1] = ADSL_ENDIAN_CONV_2SHORTS(pL[1]);
		}
#endif
			srcPtr += 3;
			tmp = ADSL_ENDIAN_CONV_LONG(*srcPtr++);
#ifdef FLATTEN_ADDR_ADJUST
#ifdef CHECK_LMEM_ADDR_IN_STAT
			...
			break;
		case kDslPwrMgrStatus:
			status->param.dslPwrMsg.msgType = ADSL_ENDIAN_CONV_LONG(*srcPtr++);
			status->param.dslPwrMsg.param.msg.msgLen = ADSL_ENDIAN_CONV_LONG(*srcPtr++);
			tmp = ADSL_ENDIAN_CONV_LONG(*srcPtr++);
#ifdef FLATTEN_ADDR_ADJUST
#ifdef CHECK_LMEM_ADDR_IN_STAT
			if(ADSL_MIPS_LMEM_ADDR(tmp))
...
#endif
		case kDslAfeTestStatus:
			status->param.dslAfeTestStatus.type =  ADSL_ENDIAN_CONV_LONG(*srcPtr++);
			status->param.dslAfeTestStatus.param1 =  ADSL_ENDIAN_CONV_LONG(*srcPtr++);
			status->param.dslAfeTestStatus.param2 =  ADSL_ENDIAN_CONV_LONG(*srcPtr++);
			break;

		case kDslCommandBufferChange:
		case kDslStatusBufferChange:
			tmp = ADSL_ENDIAN_CONV_LONG(*srcPtr++);
			status->param.value = (long)(ADSL_ADDR_TO_HOST(tmp));
			break;
...
#ifdef SUPPORT_STATUS_BACKUP
Public int	BackUpFlattenBufferStatusRead(stretchBufferStruct *fBuf, dslStatusStruct *status)
{
	ulong	*p;
	int		statBytesAvail, n;
...
Public int	FlattenBufferCommandWrite(stretchBufferStruct *fBuf, dslCommandStruct *cmd)
{
	...
	p = (ulong *) StretchBufferGetWritePtr(fBuf);
#ifdef FLATTEN_ADDR_ADJUST
	p	= (void *)ADSL_ADDR_TO_HOST(p);
#endif
	n = FlattenCommand (cmd, p, (ulong)statBytesAvail-4);
	if (n > 0) {
		n = (n + 3) & ~3;
		*(p + (n >> 2)) = ADSL_ENDIAN_CONV_LONG(BUFFER_ANCHOR);
