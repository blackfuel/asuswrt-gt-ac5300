...
#define VECT_SKBS_IN_POOL	12
#else
#define VECT_SKBS_IN_POOL	6
#endif
...
{
	if(NULL != vectSkbDev) {
		DiagStrPrintf(lineId, DIAG_DSL_CLIENT, "VectSkbDevInfo: numOfSkbs=%d skbBufIndex = %d extraSkb=%ld\n",
...
void * BcmXdslCreatePktHdr(unsigned char lineId, struct sk_buff **ppSkb)
{
	...
	pXdslMib = (void *) AdslCoreGetObjectValue(lineId, NULL, 0, NULL, &len);
	
	len = sizeof(vectState);
	if(kAdslMibStatusSuccess != (res = AdslCoreGetObjectValue(lineId, oidStr, sizeof(oidStr), &vectState, &len))) {
	...
	for (i = 0; i < sizeof(wanDevNames)/sizeof(wanDevNames[0]); i++) {
		...
		printk ("\tdev = %s(0x%X) macAddr=%X:%X:%X:%X:%X:%X\n",
			wanDevNames[i], (int) dev,
...
void BcmXdslSendErrorSamples(unsigned char lineId, VectorErrorSample *pVectErrorSample)
{
	...
	if(NULL == wanDev) {
		...
		if(NULL != vectSkbDev) {
			if(!DevSkbFree(vectSkbDev, 0))
				printk("%s: Still not able to free previous vectSkbDev(0x08%x). Discarding it any!\n", __FUNCTION__, (uint)vectSkbDev);
		}
		if(NULL == (vectSkbDev = DevSkbAllocate(vectSkbModel, VECT_FRAME_SIZE, VECT_SKBS_IN_POOL, 0,0, dataAlignMask, ETH_HEADER_ITU_LENGTH, 0))) {
