...
static uint16_t access_serdes_reg (bool ctrl_cmd, bool do_read, uint16_t addr, uint16_t wr_data, uint16_t mask_data)
{
    pmi_lp_3_t pmi_lp_3;

    __logDebug("ctrl_cmd:%d do_read:%d addr=0x%08x, wr_data=0x%04x mask_data=0x%04x",
                ctrl_cmd, do_read, addr, wr_data, mask_data);

    ag_drv_pmi_lp_1_set(MAPPED_ADDR(addr));
    ag_drv_pmi_lp_2_set(wr_data, mask_data);
    ag_drv_pmi_lp_0_set(1, ctrl_cmd);

    memset(&pmi_lp_3, 0, sizeof(pmi_lp_3_t));
#ifdef LP_CHECK_DONE
    __logDebug("LP_CHECK_DONE");

#ifdef LP_POLL_ENABLE
    __logDebug("LP_POLL_ENABLE");

    /* Poll serdes_pmi_lp_ack to be set */
    do
    {
        udelay(50);
        ag_drv_pmi_lp_3_get(&pmi_lp_3.pmi_lp_err, &pmi_lp_3.pmi_lp_ack, &pmi_lp_3.pmi_lp_rddata);
    } while (0 == pmi_lp_3.pmi_lp_ack);
#else //LP_POLL_ENABLE
    __logDebug("LP_FIXED_WAIT");
    udelay(1000);
    ag_drv_pmi_lp_3_get(&pmi_lp_3.pmi_lp_err, &pmi_lp_3.pmi_lp_ack, &pmi_lp_3.pmi_lp_rddata);
    if (0 == pmi_lp_3.pmi_lp_ack)
        __logNotice("pmi_lp_ack not done!");

#endif //LP_POLL_ENABLE

#else //LP_CHECK_DONE
    __logDebug("SKIP LP_CHECK_DONE");
    udelay(1000);
#endif //LP_CHECK_DONE
    ...
    {
        ag_drv_pmi_lp_3_get(&pmi_lp_3.pmi_lp_err, &pmi_lp_3.pmi_lp_ack, &pmi_lp_3.pmi_lp_rddata);
        __logDebug("cmd_read:%d do_read:%d addr=0x%08x, wr_data=0x%04x mask_data=0x%04x",
                    ctrl_cmd, do_read, addr, wr_data, mask_data);
        udelay(1000);
    }
