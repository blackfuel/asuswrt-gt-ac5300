...
uint8_t arlGetHwEngine(uint32_t hwTuple)
{
    uint8_t eng;

	if (hwTuple & ARL_HW_TUPLE_ARL_MASK)
...
uint16_t arlGetHwEntIx(uint32_t hwTuple)
{
	return (uint16_t)(hwTuple & (ARL_PKT_MAX_FLOWS-1));
}

/* This is for FHW layer */
uint16_t arlGetEntIx(uint32_t hwTuple)
{
	return ((ARL_PKT_MAX_FLOWS*arlGetHwEngine(hwTuple))+arlGetHwEntIx(hwTuple));
}
...
int arlActivate(Blog_t *blog_p)
{
    ...
    else
    {
       ...
       {
          isSwSwitched = enetIsSwSwitchPortFun(((struct net_device *)blog_p->tx.dev_p)->name);
          if ( isSwSwitched )
          {
             arlState_g.failures++;
             return ARL_ERROR;
       ...
       memcpy(e.ifname, ((struct net_device *)blog_p->tx.dev_p)->name, IFNAMSIZ);
       for (i = 0; i < ETH_ALEN; i++) {
          e.mac[i] = mcMac[5-i];
       }
       bcm_arl_process_hook_g((void *)&e);
...
void arlEnable(void)
{
    ...
    hwHooks.activate_fn = (HOOKP)arlActivate;
    ...
    hwHooks.get_hw_entix_fn = (HOOK32)arlGetHwEntIx; 
