...
#ifdef I2S_TRY_PLL_CLK
static int init_i2s_pll_based_clk(int freq);
...
struct i2s_pllclk_freq_map pll_freq_map[] =
{
  ...
  /*  Fs     mclk_sel   pad_mclk_sel   mclk_rate   pdiv   ndiv_int   ndiv_frac   mdiv */
  { 8000   ,     2,         2,             2,        1,     0x18,     0x15A08,   0x62}, /*  Req Bclk:   512,000  , Mclk: 256 * Fs */     
  { 16000  ,     1,         1,             2,        1,     0x18,     0x3F91E,   0x94}, /*  Req Bclk: 1,024,000  , Mclk: 256 * Fs */     
  { 32000  ,     0,         0,             2,        1,     0x18,     0x3F91E,   0x94}, /*  Req Bclk: 2,048,000  , Mclk: 256 * Fs */     
  { 44100  ,     0,         0,             2,        1,     0x18,     0x62B28,   0x6C}, /*  Req Bclk: 2,822,400  , Mclk: 256 * Fs */     
  { 48000  ,     0,         0,             2,        1,     0x18,     0x15A08,   0x62}, /*  Req Bclk: 3,072,000  , Mclk: 256 * Fs */     
  { 88200  ,     0,         0,             2,        1,     0x18,     0x62B28,   0x36}, /*  Req Bclk: 5,664,800  , Mclk: 256 * Fs */     
  { 96000  ,     0,         0,             2,        1,     0x18,     0x9374C,   0x32}, /*  Req Bclk: 6,144,000  , Mclk: 256 * Fs */     
  { 176400 ,     0,         0,             2,        1,     0x19,     0x49E88,   0x1C}, /*  Req Bclk: 11,289,600 , Mclk: 256 * Fs */     
  { 192000 ,     0,         0,             2,        1,     0x19,     0x8F1D4,   0x1A}, /*  Req Bclk: 12,288,000 , Mclk: 256 * Fs */     
  { 384000 ,     0,         0,             1,        1,     0x19,     0x8F1D4,   0x1A}, /*  Req Bclk: 24,576,000 , Mclk: 128 * Fs */     
#endif
  { 0      ,     0,         0,             0,        0,        0,           0,      0},                                                  
};
#endif
...
static int init_i2s_pll( struct i2s_pllclk_freq_map * freq_map_ptr )
{
   ...
   printk("I2S_PLL NOT Locked!! stat:0x%08x\n", (unsigned int)val);

   return (-EFAULT);

}

struct i2s_pllclk_freq_map * i2s_get_pll_freq_map( unsigned int frequency )
{
...
static int init_i2s_pll_based_clk(int freq)
{
   ...
   /* Based on sampling frequency, choose MCLK and *
    * select divide ratio for required BCLK        */
   freq_map_ptr = i2s_get_pll_freq_map( freq );
   
   if(freq_map_ptr && (init_i2s_pll(freq_map_ptr) == 0))
   {
      I2S->cfg &= I2S_MCLK_CLKSEL_CLR_MASK;
      I2S->cfg |= freq_map_ptr->mclk_rate << I2S_MCLK_RATE_SHIFT;
...
#ifdef I2S_TRY_PLL_CLK
      /* Try to initialize i2s clocks using pll */
      ret = init_i2s_pll_based_clk(freq);
#endif      
...
#ifdef I2S_TRY_PLL_CLK
            /* Try to initialize i2s clocks using pll */
            ret = init_i2s_pll_based_clk(target_freq);
#endif    
                                  ...
                                  size_t count, loff_t *ppos)
{
   ...
   if( module_refcount(THIS_MODULE) || i2s_write_in_progress )
   {
      printk(KERN_ERR "%s: Cannot set frequency, module in use\n", __FUNCTION__);
      return -EFAULT;
   }
   ...
   if( freq != target_freq )
   {
      printk(KERN_ERR "%s: Invalid freq: %d\n", __FUNCTION__, freq);
   }
