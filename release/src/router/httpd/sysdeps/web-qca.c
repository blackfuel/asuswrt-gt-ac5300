...
char *getAPPhyMode(int unit)
{
	...
	*p = '\0';
	sep = "11";
	for (q = &mode_tbl[0]; len > 0 && m > 0 && q->mask; ++q) {
		if (!(m & q->mask))
			continue;
...
unsigned int getAPChannel(int unit)
{
	...
	snprintf(buf, sizeof(buf), "iwconfig %s", get_wifname(unit));
	fp = popen(buf, "r");
	if (fp) {
...
ej_wl_control_channel(int eid, webs_t wp, int argc, char_t **argv)
{
	...
	ret = websWrite(wp, "[\"%d\", \"%d\", \"%d\"]", channel_24, channel_50, channel_5G2);
...
static int ej_wl_channel_list(int eid, webs_t wp, int argc, char_t **argv, int unit)
{
	...
	if (band != 0 && band != 1 && band != 2) return retval;
