...
GetPhyStatus(int verbose)
{
	...
	int ext = 0;
#endif
	int i, ret, lret=0, ext_lret=0, model, mask;
	char out_buf[64];
#ifdef HND_ROUTER
...
#if defined(RTCONFIG_EXT_RTL8365MB) || defined(RTCONFIG_EXT_RTL8370MB)
	if (ext) {
		ext_lret = ext_rtk_phyState(verbose, PStatus);
		lret |= ext_lret;
	}
#endif
...
setWlOffLed(void)
{
	...
	switch(model) {
		...
		case MODEL_GTAC5300:
		{
			...
			if (wlon_unit != 1 && wlon_unit_sec != 1)
				eval("wl", "-i", "eth7", "ledbh", "9", "0");	// wl 2G
			if (wlon_unit != 2 && wlon_unit_sec != 2)
...
#if defined(RTAC3200) || defined(RTAC5300) || defined(GTAC5300)
#ifdef GTAC5300
				if (!strcmp(wif, "eth6") && (apinfos[i].ctl_ch > 48))
#else
...
GEN_CONF:
#endif
#if defined(RTCONFIG_BCMWL6) && !(defined(HND_ROUTER) && defined(RTCONFIG_HNDMFG))
		wl_check_chanspec();
#endif
