...
#else
	if (nvram_invmatch("cp_dns1", "0.0.0.0") && nvram_invmatch("cp_dns1", "")) {
		fprintf(fp, "dns1 %s\n", nvram_get("chilli_dns1"));
		if (nvram_invmatch("sv_localdns", "0.0.0.0")
		    && nvram_invmatch("sv_localdns", ""))
			fprintf(fp, "dns2 %s\n", nvram_get("sv_localdns"));
...
{
	char ver[8];
	snprintf(ver, sizeof(ver), "%d", EXTEND_AIHOME_API_LEVEL);
	f_write_string(APP_API_LEVEL_PATH, ver, 0, 0644);
}
...
//		regen_cifs_config_file();
	...
	else if (strcmp(script, "vpnc") == 0)	//Activate/inactivate a VPN profile
	{
		...
		if(action & RC_SERVICE_STOP)
		{
			_dprintf("[%s, %d]stop vpnc\n", __FUNCTION__, __LINE__);
			stop_vpnc_by_unit(vpnc_unit);
		}

		if(action & RC_SERVICE_START)
		{
			_dprintf("[%s, %d]start vpnc\n", __FUNCTION__, __LINE__);
