 	...
 	if ((fp = fopen("/proc/net/dev", "r")) != NULL)
	{
		...
		while (fgets(buf, sizeof(buf), fp))
		{
			...
			*p = 0;
			...
			// debug log
			TL_LOG("tx/tx_t/tx_n = %16llu/%16lld/%16lld", tx, tx_t, tx_n);
			TL_LOG("rx/rx_t/rx_n = %16llu/%16lld/%16lld", rx, rx_t, rx_n);

			if(match) break;
		}
		fclose(fp);
	}
	TL_LOG("interface=%s, ifmap=%s, result=%lld", interface, ifmap, result);
...
void traffic_limiter_WanStat(char *buf, char *ifname, char *start, char *end, int unit)
{
	...
	TL_LOG("ifname=%s, ifmap=%s, start=%s, end=%s, unit=%d\n", ifname, ifmap, start, end, unit);
	...
	if ( ((strstr(wan_if, "none") && strstr(wan_if, "wan")) || (strstr(wan_if, "usb") && strstr(wan_if, "wan")))
		&& !strcmp(ifmap, "wan") )
		divide = 2;
	TL_LOG("wan_if=%s, ifmap=%s, divide=%d\n", wan_if, ifmap, divide);
#endif
	...
	if ((te - now) < DAY && (te - now) > 0) {
		...
		snprintf(cmd, sizeof(cmd), TL_PATH"%s/tmp", ifmap);
		snprintf(tmp, sizeof(tmp), "%lld", current);
		f_write_string(cmd, tmp, 0, 0);
		TL_DBG("Real-Time UPDATE /jffs/tld/%s/tmp, current=%lld\n", ifmap, current);
	}
	...
	if (sql_get_table(db, sql, &result, &rows, &cols) == SQLITE_OK)
	{
		...
		for (i = 0; i < rows; i++) {
			for (j = 0; j < cols; j++) {
				...
				snprintf(buf, TL_BUFSIZE, "[%llu]", (tx + rx + current)/divide);
				++index;
			}
			// debug usage
	...
	TL_LOG("ifname=%s, start=%s, end=%s, current=%lld, divide=%d, buf=%s\n", ifname, start, end, current, divide, buf);
	if (!strcmp(buf, "")) {
		if(current != 0) 
			snprintf(buf, TL_BUFSIZE, "[%lld]", current);
		else
			snprintf(buf, TL_BUFSIZE, "[0]");
	}
...
void ifname_mapping(char *word, char *ifname)
{
	if (!strcmp(word, "usb") && nvram_get_int("usb_modem_act_imsi"))
		snprintf(ifname, IFNAME_MAX, nvram_safe_get("usb_modem_act_imsi"));
	else
		snprintf(ifname, IFNAME_MAX, word);
}
