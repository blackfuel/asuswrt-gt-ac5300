...
void traffic_limiter_datawrite(char *dualwan_mode)
{
	...
	int lock;	// file lock
	int ret;
	int is_first = 0; // check is_first to decide to save database or not
	...
	for (unit = TL_UNIT_S; unit < TL_UNIT_E; ++unit)
	{
		...
		if (!f_exists(path)) {
			...
			eval("touch", path);
			is_first = 1;
			TL_DBG("IS_FIRST not to save database\n");
		}
		...
		if (f_exists(path)) {
			...
			snprintf(sql, sizeof(sql), "DELETE FROM traffic WHERE timestamp NOT BETWEEN %ld AND %ld", (date_start - DATA_PERIOD - 30), (now + 30));
		...
		if ((fp = fopen("/proc/net/dev", "r")) != NULL)
		{
			...
			while (fgets(buf, sizeof(buf), fp))
			{
				...
				*p = 0;
				...
				snprintf(tmp, sizeof(tmp), "%llu", tx);
				f_write_string(tx_path, tmp, 0, 0);

				// individual interface rx_t
				snprintf(tmp, sizeof(tmp), "%llu", rx);
				...
				// not to save database when enable function at first time
				if (is_first == 0) {
					snprintf(sql, sizeof(sql), "INSERT INTO traffic VALUES ('%lu','%s','%llu','%llu')",
						now, ifmap, tx_n, rx_n);
...
void traffic_limiter_dataread(char *q_if, char *q_ts, char *q_te)
{
	...
	te = atoll(q_te);
	ts = atoll(q_ts);
	snprintf(sql, sizeof(sql), "SELECT ifname, SUM(tx), SUM(rx) FROM (SELECT timestamp, ifname, tx, rx FROM traffic WHERE timestamp BETWEEN %ld AND %ld) WHERE ifname = \"%s\"",
		(ts - 30), (te + 30), ifmap);
...
void traffic_limiter_queryreal(char *dualwan_mode)
{
	...
	char *g, *word, *dualwan; // wan / lan / usb
	char buf[64], end[32];
	char tmp[64], prefix[] = "tlXXXXXXXX_";
	char path[IFPATH_MAX];
	...
	for (unit = TL_UNIT_S; unit < TL_UNIT_E; ++unit)
	{
		...
		time(&now);
		snprintf(end, sizeof(end), "%ld", now + 30); // for get realtime, to add 30 secs
		...
		TL_DBG("unit=%d, new=%s, real=%.6f, ls=%d, lv=%.6f, as=%d, av=%.6f\n", unit, new, real, limit_s, limit_v, alert_s, alert_v);
...
void traffic_limiter_HW_reboot(char *dualwan_mode)
{
	...
	for (unit = TL_UNIT_S; unit < TL_UNIT_E; ++unit)
	{
		...
		if (current)
		{
			...
			// get timestamp
			time(&now);
			snprintf(sql, sizeof(sql), "INSERT INTO traffic VALUES ('%lu','%s','0','%llu')", now, ifmap, current);
			sqlite3_exec(db, sql, NULL, NULL, &zErr);
			if (zErr != NULL) sqlite3_free(zErr);

			f_write_string(rx_path, "0", 0, 0);

			snprintf(rx_path, sizeof(rx_path), "%s/tx_t", tmp_t);
			f_write_string(rx_path, "0", 0, 0);

			snprintf(rx_path, sizeof(rx_path), "%s/rx_t", tmp_t);
			f_write_string(rx_path, "0", 0, 0);
